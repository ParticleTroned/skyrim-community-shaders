...
namespace
{
	// This branch intentionally ships an English-only UI. Keep the upstream call
	// sites readable without introducing the translation subsystem or resources.
	constexpr const char* TKEY(const char* a_key) noexcept { return a_key; }
	constexpr const char* T(const char*, const char* a_fallback) noexcept { return a_fallback; }
...
void WeatherPicker::RenderWeatherControls(RE::Sky* sky)
{
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		Util::DrawMultiLineTooltip({ "Previews the selected weather immediately without installing a persistent weather override.",
			"The climate may replace an unlocked preview quickly.",
			"Weather Lock continues to hold the selected weather." });
	}
