...
namespace
{
	...
	bool ApplyBasePresetJson(const json& a_json, AdaptiveBalance::Settings& a_settings, std::string& o_status)
	{
		...
		try {
			...
			a_settings.dayStartHour = WrapHour(GetOptionalFloat(*presetJson, "dayStartHour", a_settings.dayStartHour));
			a_settings.nightStartHour = WrapHour(GetOptionalFloat(*presetJson, "nightStartHour", a_settings.nightStartHour));
			a_settings.transitionHours = std::clamp(SafeFinite(GetOptionalFloat(*presetJson, "transitionHours", a_settings.transitionHours), 1.0f), 0.0f, 4.0f);
