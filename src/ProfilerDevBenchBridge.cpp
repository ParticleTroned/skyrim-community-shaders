...
namespace
{
	...
	json BuildProfilerResult(const json& a_args)
	{
		...
		if (action == "ocu_foveation") {
			return RunOnMainThread([]() -> json {
				const auto& gi = globals::features::screenSpaceGI;
				return {
					{ "action", "ocu_foveation" }, { "vr", globals::game::isVR },
					{ "loaded", gi.loaded }, { "enabled", gi.settings.Enabled },
					{ "requested", gi.settings.ExperimentalOCUEffectFoveation },
					{ "active", gi.ocuEffectActive.load(std::memory_order_relaxed) },
		...
		if (action != "status" && action != "enable" && action != "disable") {
			return {
				...
				{ "action", action },
				{ "supported", json::array({ "status", "enable", "disable", "accepted_draws", "ocu_foveation" }) },
			};
		}
		...
		DevBenchAPI::WriteFn a_write)
	{
		json result{
			...
			{ "actions", json::array({ "status", "enable", "disable", "accepted_draws", "ocu_foveation" }) },
		};
...
namespace ProfilerDevBenchBridge
{
	void Install()
	{
		...
		static constexpr const char* descriptor =
			R"({"description":"Inspect and control the CSX GPU/CPU profiler. accepted_draws reports VR accepted-draw API readiness, callbacks, isolated replays and faults without enabling capture. ocu_foveation reports requested and active peripheral GI sampling and its fallback reason without changing settings or enabling capture. Timer values and statistics are self time with profiled descendants excluded; topLevelMs and resolvedTotalMs retain inclusive depth-zero GPU time. CPU totals sum CPU self time. Every response identifies the exact producing DLL. expectedBuildId makes captures fail closed when the loaded binary is not the intended build.","inputSchema":{"type":"object","properties":{"action":{"type":"string","enum":["status","enable","disable","accepted_draws","ocu_foveation"],"default":"status","description":"Status reports timingSemantics=gpu_cpu_self_time."},"expectedBuildId":{"type":"string","description":"Exact 64-character CSX Build ID required for this operation."}}}})";
