...
namespace MenuFonts
{
	...
	/** @brief RAII guard for pushing and popping an arbitrary ImGui font. */
	class ImFontGuard
	{
	public:
	...
	/**
	 * @brief Adds bounded catalog-font previews to the current ImGui atlas.
	 *
	 * Must be called after role fonts are added and before the atlas is built.
	 */
	void AddPreviewFontsToAtlas(float previewFontSize);

	/** @brief Drops preview pointers after the ImGui atlas is cleared. */
	void InvalidatePreviewFonts();

	/** @brief Returns the catalog preview font for a relative font file. */
