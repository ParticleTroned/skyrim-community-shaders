...
void main(uint3 id : SV_GroupThreadID)
{
	...
	sums0[lane] = s0; sums1[lane] = s1; sums2[lane] = s2; sums3[lane] = s3; invalidCodecs[lane] = invalid;
	GroupMemoryBarrierWithGroupSync();
	for (uint step = 128u; step > 0u; step >>= 1u) {
		if (lane < step) {
			sums0[lane] += sums0[lane + step]; sums1[lane] += sums1[lane + step];
			sums2[lane].x = max(sums2[lane].x, sums2[lane + step].x);
			sums2[lane].yzw += sums2[lane + step].yzw; sums3[lane] += sums3[lane + step];
	...
	if (lane == 0u) {
		...
		Statistics[1] = sums1[0] / valid; Statistics[2] = sums2[0]; Statistics[3] = sums3[0];
