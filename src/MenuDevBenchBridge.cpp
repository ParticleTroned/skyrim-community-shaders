...
namespace
{
	...
	json BuildStatus()
	{
		...
		return {
			...
			{ "frustumFastPath", VRFrustumFastPath::GetStatus() },
			...
			{ "nativeFrustum", VRFrustumTelemetry::GetStatus() },
	...
	json BuildResult(const json& a_args)
	{
		...
		if (action != "status" && action != "open" && action != "close" && action != "screenshot" && action != "set_path" && action != "set_layout_unlocked" && action != "texture_stats" && action != "set_depth_culling_performance_mode" && action != "set_depth_culling_legacy_mode" && action != "set_depth_culling_telemetry_enabled" && action != "set_frustum_fast_path_enabled" && action != "set_frustum_fast_path_verification" && action != "set_frustum_telemetry_enabled" && action != "set_frustum_telemetry_detail_enabled" && action != "reset_depth_culling_telemetry" && action != "set_adaptive_balance_enabled" && action != "set_foliage_lighting_enabled" && action != "set_truepbr_verbose_json_logging" && action != "set_dynamic_cubemap_resolution" && action != "prepare_coc" && action != "prepare_tuning" && action != "set_motion_adaptive_sharpening") {
			return {
				{ "error", "unknown action" },
				{ "action", action },
				{ "supported", json::array({ "status", "open", "close", "screenshot", "set_path", "set_layout_unlocked", "texture_stats", "set_depth_culling_performance_mode", "set_depth_culling_legacy_mode", "set_depth_culling_telemetry_enabled", "set_frustum_fast_path_enabled", "set_frustum_fast_path_verification", "set_frustum_telemetry_enabled", "set_frustum_telemetry_detail_enabled", "reset_depth_culling_telemetry", "set_adaptive_balance_enabled", "set_foliage_lighting_enabled", "set_truepbr_verbose_json_logging", "set_dynamic_cubemap_resolution", "prepare_coc", "prepare_tuning", "set_motion_adaptive_sharpening" }) },
			};
		}
		...
		if ((action == "set_layout_unlocked" || action == "set_depth_culling_performance_mode" || action == "set_depth_culling_legacy_mode" || action == "set_depth_culling_telemetry_enabled" || action == "set_frustum_fast_path_enabled" || action == "set_frustum_fast_path_verification" || action == "set_frustum_telemetry_enabled" || action == "set_frustum_telemetry_detail_enabled" || action == "set_adaptive_balance_enabled" || action == "set_foliage_lighting_enabled" || action == "set_truepbr_verbose_json_logging") &&
		...
		return RunOnMainThread([action, path, enabled, resolution, motionSharpening]() -> json {
			...
			if (action == "set_frustum_fast_path_enabled") {
				if (enabled && !VRFrustumFastPath::IsInstalled())
					return { { "error", "verified native frustum implementation unavailable" } };
				globals::features::vr.SetFrustumFastPathEnabled(enabled);
				return { { "action", action }, { "status", BuildStatus() } };
			}
			if (action == "set_frustum_fast_path_verification") {
				if (!VRFrustumFastPath::SetVerification(enabled))
					return { { "error", "native frustum unavailable or verification mismatch latched; restart required" } };
				return { { "action", action }, { "status", BuildStatus() } };
			}
			if (action == "set_frustum_telemetry_detail_enabled") {
				VRFrustumTelemetry::SetDetailEnabled(enabled);
				return { { "action", action }, { "enabled", enabled }, { "status", BuildStatus() } };
			}
			if (action == "set_frustum_telemetry_enabled") {
