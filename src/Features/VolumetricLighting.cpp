...
void VolumetricLighting::DataLoaded()
{
	...
	if (REL::Module::IsVR() && bDepthBufferCulling && shaderCache->IsDiskCacheActive()) {
		// The VR depth-buffer-culling workaround only needs the ImageSpace depth-copy
		// permutations refreshed. Clearing the entire cache here discards the fully
		// prepared world and feature shaders immediately before the first save load,
		// leaving them to be rebuilt piecemeal while the world is already rendering.
