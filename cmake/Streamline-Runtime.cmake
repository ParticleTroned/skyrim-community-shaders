...
option(
    ...
    "Explicitly download and stage the hash-pinned official Streamline SDK runtime"
    OFF
)
...
if(CSX_STAGE_LOCAL_DLSS_RUNTIME AND CSX_FETCH_OFFICIAL_STREAMLINE_RUNTIME)
    message(
        FATAL_ERROR
        "CSX_STAGE_LOCAL_DLSS_RUNTIME and CSX_FETCH_OFFICIAL_STREAMLINE_RUNTIME are mutually exclusive"
    )
endif()
...
if(NOT CSX_STAGE_LOCAL_DLSS_RUNTIME AND
   NOT CSX_FETCH_OFFICIAL_STREAMLINE_RUNTIME)
    # Switching a configured tree back to source-only mode must not retain a
    # previously staged private runtime payload or its absolute-path manifest.
    ...
    set(STREAMLINE_RUNTIME_NOTICE_SOURCE "")
    set(STREAMLINE_RUNTIME_NOTICE_SHA256 "")
    set(STREAMLINE_RUNTIME_NOTICE_DEPENDENCIES "")
    message(STATUS "Streamline runtime staging is disabled")
    return()
endif()

if(CSX_FETCH_OFFICIAL_STREAMLINE_RUNTIME)
    include("${CMAKE_CURRENT_LIST_DIR}/Streamline-Official-Runtime.cmake")
    set(
        STREAMLINE_RUNTIME_PACKAGED_FILENAMES
        ${STREAMLINE_NORMAL_DLSS_RUNTIME_FILENAMES}
    )
    set(STREAMLINE_RUNTIME_SOURCE_DIRECTORY
        "${STREAMLINE_OFFICIAL_RUNTIME_DIRECTORY}"
    )
    set(STREAMLINE_RUNTIME_PACKAGE_SUFFIX "")
    set(STREAMLINE_RUNTIME_NOTICE_SOURCE "")
    set(STREAMLINE_RUNTIME_MODE_DESCRIPTION
        "official Streamline ${CSX_OFFICIAL_STREAMLINE_VERSION} runtime"
    )
else()
    set(
        STREAMLINE_RUNTIME_PACKAGED_FILENAMES
...
function(csx_stage_streamline_runtime _filename)
    if(_filename STREQUAL "nvngx_dlssnr.dll")
        ...
        set(_source "${STREAMLINE_RUNTIME_SOURCE_DIRECTORY}/${_filename}")
    endif()
    if(NOT EXISTS "${_source}")
        message(FATAL_ERROR "Required Streamline runtime is missing: ${_source}")
    endif()
...
set(STREAMLINE_RUNTIME_NOTICE_SHA256 "")
set(STREAMLINE_RUNTIME_NOTICE_VERIFY_SCRIPT
    "set(STAGE_PINNED_RUNTIME_NOTICE_DESTINATION [==[${STREAMLINE_RUNTIME_FEATURE_ROOT}]==])\n"
)
set(STREAMLINE_RUNTIME_NOTICE_INSTALL_SCRIPT
    "set(STAGE_PINNED_RUNTIME_NOTICE_DESTINATION \"\${CMAKE_INSTALL_PREFIX}\")\n"
)
set(STREAMLINE_RUNTIME_NOTICE_BYPRODUCTS "")
set(STREAMLINE_RUNTIME_NOTICE_DEPENDENCIES "")
if(NOT "${STREAMLINE_RUNTIME_NOTICE_SOURCE}" STREQUAL "")
    ...
    string(APPEND STREAMLINE_RUNTIME_NOTICE_VERIFY_SCRIPT
    ...
    string(APPEND STREAMLINE_RUNTIME_NOTICE_INSTALL_SCRIPT
        "set(STAGE_PINNED_RUNTIME_NOTICE_SOURCE [==[${STREAMLINE_RUNTIME_NOTICE_SOURCE}]==])\nset(STAGE_PINNED_RUNTIME_NOTICE_SHA256 [==[${STREAMLINE_RUNTIME_NOTICE_SHA256}]==])\n"
    )
    set(STREAMLINE_RUNTIME_NOTICE_BYPRODUCTS
...
file(
    ...
    CONTENT
        "set(STAGE_PINNED_RUNTIME_MANIFEST [==[${STREAMLINE_RUNTIME_MANIFEST}]==])\nset(STAGE_PINNED_RUNTIME_MANIFEST_SHA256 [==[${STREAMLINE_RUNTIME_MANIFEST_SHA256}]==])\nset(STAGE_PINNED_RUNTIME_DESTINATION \"\${CMAKE_INSTALL_PREFIX}/Shaders/Upscaling/Streamline\")\n${STREAMLINE_RUNTIME_NOTICE_INSTALL_SCRIPT}include([==[${CMAKE_CURRENT_LIST_DIR}/StagePinnedLocalRuntime.cmake]==])\n"
)
add_custom_target(
    ...
    BYPRODUCTS
        ${STREAMLINE_RUNTIME_FILES}
        ${STREAMLINE_RUNTIME_NOTICE_BYPRODUCTS}
        "${STREAMLINE_RUNTIME_VERIFY_STAMP}"
    DEPENDS
        ${STREAMLINE_RUNTIME_SOURCE_FILES}
        "${STREAMLINE_RUNTIME_MANIFEST}"
        "${STREAMLINE_RUNTIME_VERIFY_SCRIPT}"
        "${STREAMLINE_RUNTIME_INSTALL_SCRIPT}"
        ${STREAMLINE_RUNTIME_NOTICE_DEPENDENCIES}
        "${CMAKE_CURRENT_LIST_DIR}/StagePinnedLocalRuntime.cmake"
    COMMENT "Verifying and staging the ${STREAMLINE_RUNTIME_MODE_DESCRIPTION}"
    VERBATIM
)
if(TARGET ${PROJECT_NAME})
    add_dependencies(${PROJECT_NAME} VerifyStreamlineRuntime)
endif()

message(
    STATUS
    "Staged the ${STREAMLINE_RUNTIME_MODE_DESCRIPTION}"
)
