...
> **Superseded as qualification evidence.** This capture remains authoritative
> for scene provenance, the native structured visibility-resource contract,
> and the observed stereo draw route. A later PR5 review demonstrated that the
> implementation and graph builder used here could accept an intervening slot
> overwrite or a producer/view resource mismatch. Its broad GPU query span also
> included unrelated work. Accordingly, its current-frame-consumption and
> performance conclusions are historical hypotheses to retest against the
> corrected final-draw implementation, not release evidence.
