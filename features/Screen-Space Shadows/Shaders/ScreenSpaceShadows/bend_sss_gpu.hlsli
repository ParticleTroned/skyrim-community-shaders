...
void WriteScreenSpaceShadow(DispatchParameters inParameters, int3 inGroupID, int inGroupThreadID)
{
	...
	[unroll] for (i = 0; i < READ_COUNT; i++)
	{
		...
		SSS_COORD2 coord = read_xy * inParameters.InvDepthTextureSize * inParameters.DynamicRes;
		SSS_COORD2 coord_with_offset = (read_xy + offset_xy) * inParameters.InvDepthTextureSize * inParameters.DynamicRes;

#if defined(VR)
		// VR side-by-side: halve x to map stereo pixel coords to texture UV.
		coord *= SSS_COORD2(0.5, 1.0);
		coord_with_offset *= SSS_COORD2(0.5, 1.0);
