...
namespace
{
	...
	ImDrawData* FilterShaderCompilationWindowFromHMD(ImDrawData* drawData, ImDrawData& filteredDrawData)
	{
		...
		const bool hideSoftwareCursorForeground = ImGui::GetIO().MouseDrawCursor;
		...
		for (int i = 0; i < drawData->CmdListsCount; ++i) {
			...
			if ((hideShaderCompilationWindow && IsDrawListOwnedByWindow(cmdList, "ShaderCompilationInfo")) ||
				(hideSoftwareCursorForeground && IsDrawListOwnedByWindow(cmdList, "##Foreground"))) {
				removedAny = true;
				continue;
			}
	...
	bool ShouldDrawCustomVRCursorDot(const ImGuiIO& io)
	{
		if (!io.MouseDrawCursor ||
			!std::isfinite(io.MousePos.x) ||
			!std::isfinite(io.MousePos.y) ||
			io.MousePos.x < 0.0f ||
			io.MousePos.y < 0.0f ||
...
void VR::SubmitOverlayFrame()
{
	...
	if (shouldRenderOverlay && wantsAnyVROverlay && (useInSceneOverlay || canUseIVROverlay) && hasRequiredTextures) {
		...
		ImGuiIO& io = ImGui::GetIO();
		const bool useCustomVRCursorDot = io.MouseDrawCursor;
		...
		auto renderImGuiToTexture = [&](ID3D11RenderTargetView* targetRTV) {
			...
			if (useCustomVRCursorDot && ShouldDrawCustomVRCursorDot(io) && io.Fonts && io.Fonts->IsBuilt()) {
				...
				AppendCustomVRCursorDot(cursorDrawList, io.MousePos);
