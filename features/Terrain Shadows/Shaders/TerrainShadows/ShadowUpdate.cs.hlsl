...
{
	// Signed modulus is required here: coord can be negative while wrapping
	// coordinates off either edge, and that case is handled below.
...
[numthreads(NTHREADS, 1, 1)] void main(const uint gtid : SV_GroupThreadID, const uint gid : SV_GroupID) {
	...
	// Derive UAV coordinates in integer pixel space so adjacent groups cannot race at UV boundaries.
