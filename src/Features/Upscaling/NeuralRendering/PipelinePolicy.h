...
namespace NeuralRendering
{
	...
	/** Attributes any valid per-eye or stereo Feature 18 slot subset to its route. */
	...
	static_assert(ClassifyFeatureSlotMask(0b0001u) == FeatureSlotRoute::Main);
	static_assert(ClassifyFeatureSlotMask(0b0010u) == FeatureSlotRoute::Main);
	static_assert(ClassifyFeatureSlotMask(0b0011u) == FeatureSlotRoute::Main);
	static_assert(ClassifyFeatureSlotMask(0b0100u) == FeatureSlotRoute::Submit);
	static_assert(ClassifyFeatureSlotMask(0b1000u) == FeatureSlotRoute::Submit);
	static_assert(ClassifyFeatureSlotMask(0b1100u) == FeatureSlotRoute::Submit);
	static_assert(ClassifyFeatureSlotMask(0u) == FeatureSlotRoute::Unexpected);
	static_assert(ClassifyFeatureSlotMask(0b0101u) == FeatureSlotRoute::Unexpected);
	static_assert(IsOrderedStereoFeatureSlotPair(0u, 1u));
	static_assert(IsOrderedStereoFeatureSlotPair(2u, 3u));
	static_assert(!IsOrderedStereoFeatureSlotPair(1u, 0u));
	static_assert(!IsOrderedStereoFeatureSlotPair(0u, 2u));
		...
		bool a_directCommit) noexcept
	{
		if (a_batchedStereo)
			return a_directCommit ?
			           "batched_direct" :
			           "batched_staged";
		return a_directCommit ? "per_eye_direct" : "per_eye_staged";
	}
