...
namespace
{
  ...
  "inputSchema":{
    ...
    "properties":{
      ...
      "mode":{"oneOf":[{"type":"string","enum":["full_resolution","foveated","reduced_resolution"]},{"type":"integer","minimum":0,"maximum":2}],"description":"NR execution mode: full output resolution, shared FOV masks, or reduced input before DLSS upscaling. Character isolation can combine with every mode. Enabling foveated mode requires configured and active FOV in Upscaling."},
      "fovOnly":{"type":"boolean","description":"Limit full- or reduced-resolution NR to the active FOV mask. Requires configured and enabled FOV in Upscaling; full resolution reuses shared geometry, offsets and feather, including the FOV+TAA outer boundary."},
	...
	json NeuralRenderingStatusJson(const Upscaling& a_upscaling)
	{
		...
		const bool fovRequired = NeuralRendering::RequiresFoveatedMask(a_upscaling.GetNeuralRenderingMode(), a_upscaling.settings.neuralRenderingFovOnly);
		...
		return {
			...
			{ "runtimeSupport", {
									...
									{ "reducedResolution", globals::game::isVR },
									{ "character", globals::game::isVR },
									{ "requestedConfiguration", NeuralRendering::IsRenderingConfigurationSupported(globals::game::isVR,
																	a_upscaling.GetNeuralRenderingMode(), a_upscaling.settings.neuralCharacterRenderingEnabled) },
									{ "reason", "Foveated, reduced-resolution, and character routes require Skyrim VR; full-resolution supports SE/AE/VR" },
								} },
...
#	undef QUALIFICATION_DELTA
	...
	struct NeuralRenderingConfigurationRequest
	{
		...
		{
			return enabled || mode || fovOnly || insertionPoint || preset || HasImageTuningOverrides() ||
		...
		if (action == "nr_readiness") {
			...
			return RunOnMainThread([]() {
				...
				require(NeuralRendering::IsRenderingConfigurationSupported(globals::game::isVR,
							upscaling.GetNeuralRenderingMode(), upscaling.settings.neuralCharacterRenderingEnabled),
					"rendering_configuration_unsupported_runtime");
				require(!NeuralRendering::RequiresFoveatedMask(upscaling.GetNeuralRenderingMode(), upscaling.settings.neuralRenderingFovOnly) ||
		...
		if (action == "nr_configure") {
			...
			return RunOnMainThread([request]() {
				...
				if (requestedSettings.neuralRenderingEnabled &&
					...
					return json{
						{ "error", "Foveated, reduced-resolution, and character routes require Skyrim VR; full-resolution supports SE/AE/VR" },
						{ "errorCode", "unsupported_runtime" },
						{ "action", "nr_configure" },
				...
				if (requestedSettings.neuralRenderingEnabled &&
					NeuralRendering::RequiresFoveatedMask(NeuralRendering::ClampRenderingMode(requestedSettings.neuralRenderingMode), requestedSettings.neuralRenderingFovOnly) &&
					!upscaling.IsNeuralRenderingFovConfigurationAvailable()) {
					return json{
						{ "error", "Configure and enable FOV in Upscaling before using FOV-dependent NR" },
