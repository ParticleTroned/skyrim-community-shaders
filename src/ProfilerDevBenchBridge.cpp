...
namespace ProfilerDevBenchBridge
{
	void Install()
	{
		...
		static constexpr const char* descriptor =
			R"({"description":"Inspect and control the CSX GPU/CPU profiler. Status requests one capture and returns the latest resolved timers, captured frame, slot counters and gpu_cpu_self_time semantics. GPU totals retain depth-zero inclusive time; CPU totals sum self time. Every response identifies the producing build. expectedBuildId rejects a different loaded build.","inputSchema":{"type":"object","properties":{"action":{"type":"string","enum":["status","enable","disable"],"default":"status"},"expectedBuildId":{"type":"string"}}}})";
