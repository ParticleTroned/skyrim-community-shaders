...
int main()
{
	...
	counters.RecordBindAttempt(CSX::VRDepthCullingDiagnostics::DrawCategory::Lighting);
	counters.RecordDescriptorMismatch(CSX::VRDepthCullingDiagnostics::DrawCategory::Lighting);
	...
	if (!snapshot.collecting ||
		...
		snapshot.bindAttempts != 4 || snapshot.boundDraws != 1 ||
		...
		snapshot.bindAttemptsByCategory[CSX::VRDepthCullingDiagnostics::ToIndex(
			CSX::VRDepthCullingDiagnostics::DrawCategory::Lighting)] != 1 ||
		...
		snapshot.failOpenByCategory[CSX::VRDepthCullingDiagnostics::ToIndex(
			CSX::VRDepthCullingDiagnostics::DrawCategory::Lighting)]
								   [CSX::VRDepthCullingDiagnostics::ToIndex(
									   CSX::VRDepthCullingDiagnostics::FailOpenReason::DescriptorMismatch)] != 1) {
		return 2;
	}

	counters.Stop();
	counters.RecordBindAttempt();
	if (counters.Capture().bindAttempts != 4)
		return 3;

	CSX::VRDepthCullingDiagnostics::Counters epochCounters;
	epochCounters.Start();
	const auto staleEpoch = epochCounters.CollectionEpoch();
	epochCounters.RecordPipelineTiming(staleEpoch, 99);
	epochCounters.Reset();
	epochCounters.RecordPipelineTiming(staleEpoch, 101);
	const auto epochSnapshot = epochCounters.Capture();
	if (!epochSnapshot.collecting || epochSnapshot.collectionEpoch == staleEpoch ||
		epochSnapshot.pipelineTimingSamples != 0 || epochSnapshot.pipelineRegionNanoseconds != 0) {
		return 4;
	}
	epochCounters.Stop();
