...
int main()
{
	...
	Settings s{};
	s.detailStrength = std::numeric_limits<float>::quiet_NaN(); Require(!Valid(s));
	s = {}; s.appearanceMix = 1.01f; Require(!Valid(s));
	Profile p{};
	p.transform = Transform::ReversibleProxy; Require(!Valid(p));
	p.domain = Domain::Linear; Require(Valid(p));
	for (auto transform : { Transform::Identity, Transform::LinearToSRGB, Transform::ReversibleProxy }) {
		...
		for (float exposure : { 0.0625f, 1.0f, 4.0f }) {
			...
			for (float x : { 0.0f, 0.00001f, 0.003f, 0.04f, 0.18f, 0.5f, 1.0f, 4.0f }) {
				...
				Require(Forward(b, p, prepared)); Require(Inverse(prepared, p, inverse));
	...
	s = {}; Require(DetailGain(2.0f, 2.0f, 1.0f, s) == 1.0f);
	Require(DetailGain(1.0f, 0.0f, 0.0f, s) == 1.0f);
	s.detailStrength = 0.0f; Require(DetailGain(2.0f, 0.0f, 1.0f, s) == 1.0f);
	s.detailStrength = 2.0f; Require(DetailGain(8.0f, 0.0f, 1.0f, s) <= 2.0f);
	Configuration a{}, b{};
	b.settings.mode = Mode::PreserveSource;
	Require(!ChangesInput(a, b, 0));
	b.experiments.profiles[0] = p; Require(ChangesInput(a, b, 0)); Require(!ChangesInput(a, b, 1));
	a = b; b.settings.detailStrength = 0.2f; Require(!ChangesInput(a, b, 0));
	b.experiments.transportBypass = true; Require(ChangesInput(a, b, 0)); Require(ChangesInput(a, b, 1));
