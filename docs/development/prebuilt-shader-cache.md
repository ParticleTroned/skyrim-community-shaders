...
The tool refuses to replace arbitrary directories. It also preserves the old
runtime directory if publishing its replacement fails.
...
Reusing a label intentionally replaces the archive of that name.
...
and rollback cache directories are deleted or swapped.
