...
#				if defined(DEBUG_WETNESS_EFFECTS)
#					define CS_DEBUG_WETNESS
#				endif
#			endif
...
#			endif
#			if defined(VR) && (defined(WATER_PARALLAX) || defined(LIGHT_LIMIT_FIX))
...
#			if defined(VR) && (defined(WATER_PARALLAX) || defined(LIGHT_LIMIT_FIX))
float GetVRWaterFoveatedDetailWeight(float foveationMode, float2 eyeUv, uint eyeIndex)
{
...
{
	return GetVRWaterFoveatedDetailWeight(SharedData::VRFoveationModes.y, eyeUv, eyeIndex);
}
#			endif
...
#				if defined(FLOWMAP)
		...
		float2 flowOffset = CS_GET_FLOW_AWARE_RIPPLE_OFFSET(
...
#				endif
		raindropInfo = CS_GET_RAIN_DROPS(rippleWPosition + FrameBuffer::CameraPosAdjust[eyeIndex].xyz, CS_WETNESS_SETTINGS.Time, finalNormal, rippleStrengthModifier);
...
bool IsUnifiedWaterDepthPixelInEyeBounds(
	int2 pixelPosition,
	uint eyeIndex,
	int2 renderDimensions)
{
...
float GetUnifiedWaterShallowFallbackAvailability(float viewDistance)
{
	float maximumDistance = max(
		SharedData::unifiedWaterSettings.ShallowFallbackMaxDistance,
		0.0);
	if (!isfinite(viewDistance) || !isfinite(maximumDistance) || maximumDistance <= 0.0)
		return 0.0;

	float fadeRange = min(
		UnifiedWaterFallbackCullFadeRange,
		0.25 * maximumDistance);
	float fadeStart = maximumDistance - fadeRange;
...
#				if defined(VR)
	float2 clipPosition =
		Stereo::ConvertFromStereoUV(logicalUV, eyeIndex, 1) * 2.0 - 1.0;
#				else
	float2 clipPosition =
		(logicalUV * 2.0 - 1.0) * float2(1.0, -1.0);
#				endif
	float4 scenePositionH = mul(
		FrameBuffer::CameraViewProjInverse[eyeIndex],
		float4(clipPosition, rawDepth, 1.0));
...
{
	probeColumn = -1.0;
	if (!IsUnifiedWaterDepthPixelInEyeBounds(
			samplePixel,
			eyeIndex,
			renderDimensions)) {
		return false;
...
#					else
	float depthMul = CalculateDepthMultFromUV(Stereo::ConvertFromStereoUV(primaryDepthLogicalUV, eyeIndex, 1), depth, eyeIndex);
#					endif  //VR
...
#			if defined(UNIFIED_WATER_SHALLOW_FALLBACK)
	// Derivatives stay outside the candidate-only branch so both eyes get a
	// stable depth direction and world-space-to-pixel conversion.
...
#				if defined(LIGHT_LIMIT_FIX)
	uint lightCount = 0;
	const bool inWorld = (Permutation::ExtraShaderDescriptor & Permutation::ExtraFlags::InWorld) != 0;
	...
	float contactShadowDetailWeight = 1.0;
#					if defined(VR)
	contactShadowDetailWeight = GetVRWaterFoveatedDetailWeight(
		SharedData::VRFoveationData0.w,
		saturate(screenUV),
		eyeIndex);
	const bool contactShadowDetailEnabled = FoveatedIsShaderDetailActive(contactShadowDetailWeight);
#					else
	const bool contactShadowDetailEnabled = true;
#					endif
	[branch] if (contactShadowsEnabled && contactShadowDetailEnabled)
...
#					endif
			...
			[branch] if (
				...
				contactShadowDetailEnabled &&
				contactShadowCount > 0 &&
				HdotN > 0.0 &&
				intensityMultiplier > 1e-5 &&
				LightLimitFix::CanUseContactShadows(light, isParticleLight) &&
				LightLimitFix::IsContactShadowCandidate(clusteredLightIndex, contactShadowOffset, contactShadowCount))
			{
				if (!hasContactShadowNoise) {
					contactShadowNoise = Random::InterleavedGradientNoise(
						LightLimitFix::GetContactShadowNoiseCoord(input.HPosition.xy, screenUV, contactShadowScreenDim),
				...
				contactShadow = LightLimitFix::ContactShadows(
					...
					eyeIndex);
#					if defined(VR)
				contactShadow = lerp(1.0, contactShadow, contactShadowDetailWeight);
#					endif
			}
...
#					if defined(CS_DEBUG_WETNESS)
	...
	float3 debugColor = CS_GET_DEBUG_WETNESS_COLOR_UNDERWATER(waterData.rippleInfo, 1.5, 2.0);
	if (any(debugColor)) {
		finalColor = debugColor;
	}
#					endif
...
#					if defined(UNIFIED_WATER_SHALLOW_FALLBACK)
	...
	// can cross a bank as the view or animated normal changes.
...
#					if defined(UNIFIED_WATER_SHALLOW_FALLBACK)
	...
	// Select the contact depth from the final confidence-filtered classifier.
	// Unavailable or RC180-suppressed evidence therefore retains the exact
	// shallow range instead of leaking a remote deep-water contour into VR.
	float deepShoreDepthBlendRange = max(
		SharedData::unifiedWaterSettings.DeepShoreDepthBlendRangeUnits,
		shallowShoreDepthBlendRange);
	float selectedShoreDepthBlendRange = lerp(
		shallowShoreDepthBlendRange,
		deepShoreDepthBlendRange,
		trustedDeepContextWeight);
	float selectedShoreContactWeight =
		GetUnifiedWaterShoreContactWeight(
			waterColumnDepthUnits,
			selectedShoreDepthBlendRange,
			shoreContactDepthFootprint);
	shallowSurfaceLayerWeight =
		shallowSurfaceBaseWeight *
		selectedShoreContactWeight *
...
#						if defined(UNIFIED_WATER_SHALLOW_FALLBACK)
	...
	// refractionMul. Restore the native material colour and Fresnel only within
	// the bounded shallow layer.
...
#						if defined(UNIFIED_WATER_SHALLOW_FALLBACK)
	// Preserve the existing continuous depth/contact feather as the fallback
	// target without changing terrain or water geometry.
	finalColor = lerp(
		finalColor,
		finalColorPreFog,
		shallowSurfaceLayerWeight);
