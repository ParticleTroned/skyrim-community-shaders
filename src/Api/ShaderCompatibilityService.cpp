...
namespace
{
	...
	Status GetSnapshot(const void*, ShaderCompatibilityAPI::Snapshot001* a_output)
	{
		...
		*a_output = {
			.structSize = sizeof(*a_output), .available = 1, .phase = snapshot.phase,
			.registryRevision = snapshot.revision, .registrationCount = snapshot.registrationCount,
	...
	Status Register(const void*, const ShaderCompatibilityAPI::Registration001* a_registration, ShaderCompatibilityAPI::RegistrationReceipt001* a_output)
	{
		...
		*a_output = {
			.structSize = sizeof(*a_output), .status = result.status, .accepted = result.accepted ? 1u : 0u,
			.idempotent = result.idempotent ? 1u : 0u, .restartRequired = result.restartRequired ? 1u : 0u,
			.registrationHandle = result.handle, .registryRevision = result.revision,
			.canonicalDigest = response.digest.empty() ? nullptr : response.digest.c_str(),
			.reasonCode = response.reason.c_str(), .message = response.message.c_str()
		};
	...
	Status GetRegistration(const void*, std::uint32_t a_index, ShaderCompatibilityAPI::RegistrationDescriptor001* a_output)
	{
		...
		*a_output = {
			.structSize = sizeof(*a_output), .registrationHandle = value.handle, .identity = value.identity.c_str(),
			.owner = value.owner.c_str(), .displayVersion = value.displayVersion.c_str(),
			.contractMajor = value.contractMajor, .currentMinor = value.currentMinor,
			.minimumCompatibleMinor = value.minimumCompatibleMinor, .maximumCompatibleMinor = value.maximumCompatibleMinor,
			.resourceFingerprint = value.resourceFingerprint.c_str(), .scopeCount = static_cast<std::uint32_t>(value.scopes.size()),
