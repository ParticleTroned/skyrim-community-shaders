...
static void Require(bool value) { ++checks; if (!value) { std::fprintf(stderr, "Exposure check %u failed\n", checks); std::abort(); } }
int main()
{
	for (float average : { 0.125f, 0.5f, 1.0f, 4.0f, 16.0f }) {
		for (float target : { 0.0625f, 0.5f, 1.0f, 8.0f }) {
			...
			for (auto transform : { Transform::LinearToSRGB, Transform::ReversibleProxy }) {
				...
				Require(Valid(requested)); Require(ResolveExposureProfile(requested, value, effective));
				...
				Require(Forward(source, effective, prepared)); Require(Inverse(prepared, effective, inverse));
	...
	for (auto values : { std::array<float, 2>{0, 1}, {1, 0}, {0, 0} }) {
		...
		Require(e.validity == ExposureValidity::UnitFallback); Require(e.ratio == 1);
	...
	Require(MatchesExposure(stamp, 10, 2)); Require(!MatchesExposure(stamp, 11, 2));
	Require(!MatchesExposure(stamp, 10, 3)); stamp.ambiguous = true; Require(!MatchesExposure(stamp, 10, 2));
	Configuration a{}, b{};
	b.settings.mode = Mode::Managed; b.experiments.profiles[0] = captured;
	Require(NeedsExposureCapture(b)); Require(ChangesInput(a, b, 0)); Require(!ChangesInput(a, b, 1));
	a = b; b.experiments.applyModelEdit = false; Require(!ChangesInput(a, b, 0)); Require(b.Enabled());
	b.experiments.diagnostics = true; Require(!ChangesInput(a, b, 0));
	b.settings.enabled = false; Require(ChangesInput(a, b, 0)); Require(!NeedsExposureCapture(b));
	b.experiments.captureEngineExposure = true; Require(NeedsExposureCapture(b));
	b = {}; Require(!b.Enabled()); b.experiments.applyModelEdit = false; Require(b.Enabled());
	captured.transform = Transform::Identity; Require(!Valid(captured));
