...
This directory contains the exported measurement/review payloads, with local NR DLL paths removed,
...
The run shards total 113,194,262 bytes (about 108 MiB); the largest is under
...
Each UTF-8 JSONL line is one exported source document, with:
...
-   value: the complete original parsed JSON or text, except local NR DLL paths and explicitly
    summarized continuous recording arrays.
-   sourceNotes, when present: omitted trace-array counts or the original
    JSON parse error.

Local NR DLL paths are replaced with `[local NR DLL path removed]`,
including paths inside embedded serialized JSON. This privacy exception
applies to all formats below. Measurement values and all other payload
bytes remain unchanged. The index lists every affected record. Source
sizes and hashes identify the private originals, not sanitized exports.
...
bytes, hashes and exported values were verified at original export. The
privacy audit separately verifies that only local NR DLL paths changed.
...
validation.json retains the original export audit and records the privacy
redaction audit; complete payload-to-source equality no longer applies.
Coverage counts and measurements are unchanged. archive-index.json contains every shard hash; the source
