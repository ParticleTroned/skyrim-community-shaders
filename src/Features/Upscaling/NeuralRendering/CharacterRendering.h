...
namespace NeuralRendering
{
	...
	struct CharacterViewSnapshot
	{
		...
		/** A completed sample is available at the attributed frame and content serial. */
		bool maskCoverageReady = false;
		/** Historical samples are diagnostics only and cannot prove current emptiness. */
	...
	struct CharacterPreparedFrameSnapshot
	{
		...
		std::array<std::uint32_t, 1> sourceWorldFrames{ std::numeric_limits<std::uint32_t>::max() };
		std::array<std::uint64_t, 1> generations{};
		std::array<std::uint64_t, 1> contentSerials{};
		std::array<std::uint32_t, 1> widths{};
		std::array<std::uint32_t, 1> heights{};
		/** Expected physical evaluations: zero for bypass, one legacy, two split. */
		std::array<std::uint32_t, 1> computeRegionCounts{};
	};
	...
	struct CharacterSnapshot
	{
		...
		std::string computeRoiReason =
			"One view enclosing compute rectangle by default; opt-in experimental multi-ROI evaluates up to two disjoint character clusters independently";
		bool enabled = false;
	...
	/** Owns character observations, stable view regions, and R8 selection masks. */
	...
	public:
		...
		/** Builds an exact view mask. Failure leaves Feature 18 fail-closed. */
		bool PrepareMask(
		...
		GetDebugMaskSrv() const noexcept;
