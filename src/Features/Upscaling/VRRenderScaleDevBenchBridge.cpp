	...
	json NeuralRenderingStatusJson(const Upscaling& a_upscaling)
	{
		...
		return {
			...
			{ "runtimeSupport", {
									...
									{ "reason", "Foveated and reduced-resolution routes require Skyrim VR; full-resolution and character selection support SE/AE/VR" },
								} },
		...
		if (action == "nr_configure") {
			...
			return RunOnMainThread([request]() {
				...
				if (requestedSettings.neuralRenderingEnabled &&
					...
					return json{
						{ "error", "Foveated and reduced-resolution routes require Skyrim VR; full-resolution and character selection support SE/AE/VR" },
						{ "errorCode", "unsupported_runtime" },
						{ "action", "nr_configure" },
