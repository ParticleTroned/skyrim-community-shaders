...
namespace NeuralRendering
{
	...
	public:
		...
		struct EarlyMaskReadback
		{
			...
			std::uint32_t width = 0, height = 0, eyeCount = 0, frame = 0, categories = 0;
		...
		bool EnsureEarlyMaskBounds(EarlyMaskReadback& a_readback, ID3D11Device* a_device)
		{
			...
			if (a_readback.bounds && a_readback.width == capturedEyeWidth_ && a_readback.height == capturedHeight_ &&
				a_readback.eyeCount == capturedEyeCount_)
				return true;
			if (!capturedEyeWidth_ || !capturedHeight_ || !capturedEyeCount_ || capturedEyeCount_ > 2u ||
			...
			created.tiles.resize(static_cast<std::size_t>(columns) * rows * capturedEyeCount_);
			D3D11_BUFFER_DESC desc{};
			...
			created.eyeCount = capturedEyeCount_;
		...
		void QueueEarlyMaskBounds(ID3D11Device* a_device, ID3D11DeviceContext* a_context)
		{
			try {
				...
				for (std::size_t eye = 0; eye < capturedEyeCount_; ++eye) {
		...
		std::uint32_t capturedEyeCount_ = 0;
		...
		CharacterMaskPrepareResult& a_result) noexcept
	{
		...
		try {
			...
			if (state_->capturedFrame_ != sourceWorldFrame ||
				...
				return fail("no correlated character capture with matching logical view dimensions is available");
			}
			...
			if (!logicalEmptyCapture) {
				...
				// The capture owns the exact active-view allocation; its logical
				// eye stride remains authoritative for mono and packed stereo.
				sourceEyeWidth = state_->capturedEyeWidth_;
				if (!sourceEyeWidth || a_args.eyeIndex >= state_->capturedEyeCount_ ||
					...
					return fail(std::format(
						"character-mask capture {}x{} does not equal logical active input {}x{} per eye",
						sourceDesc.Width,
						sourceDesc.Height,
		...
		std::span<CharacterMaskPrepareResult> a_results) noexcept
	{
		...
		try {
			...
			// Validate all prepared eyes before publishing the batch. Each plan
