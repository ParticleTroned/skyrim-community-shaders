...
live-validated. The corrected depth-culling producer emits a targeted resource
version backed by an immutable COM-owned token. Geometry setup stages an exact
pass/geometry draw token; the final `Draw*` scope then saves slot 127, binds and
verifies the token view, emits and consumes the submission identity, restores
the previous slot, and disables the descriptor. Closed setup scopes and
process-global last-seen geometry are not used as proximity heuristics.
Accepted OpenVR submissions identify the final eye texture and bounds without
assuming that earlier work used two physical draws.

The guarded Bleakwind capture discovered the native structured OBB result
contract and observed 1,691 two-instance Lighting draws in one zero-drop frame.
It does not qualify the corrected path: PR5's earlier builder did not reject an
intervening slot overwrite or a version/view resource mismatch. The corrected
builder has regressions for both defects and for repeated producer epochs, but
needs a new runtime capture before the source-level chain is considered live
proved.

The next strict gate is a corrected-runtime capture proving the immutable
producer-to-final-draw bind/restore chain with zero rejected or unaccounted
bindings. After that, the remaining vertical-slice gate is the resource path
from the selected draw target to the final eye-submitted texture, plus the
selected object's full scene/material/pass identity and a generated
decision-window report. See
...
-   final-draw effective VS SRV slot verification after D3D11 hazard handling,
    including restoration of the prior slot;
