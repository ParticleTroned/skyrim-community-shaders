...
#define ENABLE_ADAPTIVE_BALANCE SharedData::linearLightingSettings.enableAdaptiveBalance
...
#	define ENABLE_ADAPTIVE_BALANCE_ADJUSTMENTS \
		(ENABLE_ADAPTIVE_BALANCE && ((Permutation::ExtraShaderDescriptor & (Permutation::ExtraFlags::InWorld | Permutation::ExtraFlags::InReflection)) != 0))
#else
#	define ENABLE_LL_COLOR_ADJUSTMENTS ENABLE_LL
#	define ENABLE_ADAPTIVE_BALANCE_ADJUSTMENTS ENABLE_ADAPTIVE_BALANCE
#endif
...
#define ENABLE_BALANCE_ADJUSTMENTS (ENABLE_LL_COLOR_ADJUSTMENTS || ENABLE_ADAPTIVE_BALANCE_ADJUSTMENTS)
...
#	endif
	...
	float3 DirectionalLight(float3 color, bool isLinear = false)
	{
		...
		} else if (ENABLE_ADAPTIVE_BALANCE_ADJUSTMENTS) {
...
#	if defined(LIGHTING)
	...
	{
		return ENABLE_BALANCE_ADJUSTMENTS ? (pow(abs(color / max(emissiveMult, 1e-5)), SharedData::linearLightingSettings.emitColorGamma) * emissiveMult * SharedData::linearLightingSettings.emitColorMult) : color;
	}
#	endif
...
#	if defined(TRUE_PBR)
		...
		return ENABLE_BALANCE_ADJUSTMENTS ? color * SharedData::linearLightingSettings.glowmapMult : color;
#	else
		return ENABLE_BALANCE_ADJUSTMENTS ? pow(abs(color), SharedData::linearLightingSettings.glowmapGamma) * SharedData::linearLightingSettings.glowmapMult : color;
#	endif
	}

	float3 Ambient(float3 color)
	{
		return ENABLE_BALANCE_ADJUSTMENTS ? pow(abs(color), SharedData::linearLightingSettings.ambientGamma) * SharedData::linearLightingSettings.ambientMult : color;
	}

	float3 Fog(float3 color)
	{
		return ENABLE_BALANCE_ADJUSTMENTS ? pow(abs(color), SharedData::linearLightingSettings.fogGamma) : color;
	}

	float FogAlpha(float alpha)
	{
		return ENABLE_BALANCE_ADJUSTMENTS ? pow(abs(alpha), SharedData::linearLightingSettings.fogAlphaGamma) : alpha;
	}

	float3 Effect(float3 color)
	{
		return ENABLE_BALANCE_ADJUSTMENTS ? pow(abs(color), SharedData::linearLightingSettings.effectGamma) : color;
	}

	float3 EffectMult(float3 color)
	{
		if (ENABLE_BALANCE_ADJUSTMENTS) {
#	if defined(MEMBRANE)
...
#	endif
	...
	{
		return ENABLE_BALANCE_ADJUSTMENTS ? SharedData::linearLightingSettings.effectLightingMult : 1.0f;
	}

	float EffectAlpha(float alpha)
	{
		return ENABLE_BALANCE_ADJUSTMENTS ? pow(abs(alpha), SharedData::linearLightingSettings.effectAlphaGamma) : alpha;
	}

	float3 Sky(float3 color)
	{
		return ENABLE_BALANCE_ADJUSTMENTS ? pow(abs(color), SharedData::linearLightingSettings.skyGamma) : color;
	}

	float3 Water(float3 color)
	{
		return ENABLE_BALANCE_ADJUSTMENTS ? pow(abs(color), SharedData::linearLightingSettings.waterGamma) : color;
	}

	float3 VolumetricLighting(float3 color)
	{
		return ENABLE_BALANCE_ADJUSTMENTS ? pow(abs(color), SharedData::linearLightingSettings.vlGamma) : color;
	}
