...
-   Requested revision: `3` for VR upscaling apply-safety calls. Revision `1` and `2` remain accepted for existing binaries, and `0` requests "latest". The bundled `GetCSInterface001()` helper requests revision `3` and returns `nullptr` on older providers so new callers do not accidentally use revision-3 methods on an older vtable.
