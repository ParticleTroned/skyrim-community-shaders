...
void CSEditor::DrawSettings()
{
	...
	if (ImGui::Button(hasCell ? "Open CS Editor" : "Open CS Editor (no active cell)", { -1, 0 })) {
		OpenEditorWindow();
	}
	ImGui::EndDisabled();

	ImGui::Spacing();
	ImGui::SeparatorText("Weather Picker");
...
void CSEditor::RenderWeatherControls(RE::Sky* sky, bool showSectionHeader)
{
	...
	ImGui::EndDisabled();
	if (auto _tt = Util::HoverTooltipWrapper()) {
		Util::DrawMultiLineTooltip({ "Instantly previews the selected weather without setting a persistent weather override.",
			"The game's normal climate can replace it quickly, so rain or storms may only last briefly.",
			"Weather Lock still holds the selected weather; leave it off to test natural expiry." });
	}
