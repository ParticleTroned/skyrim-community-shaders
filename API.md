...
-   Interface revision: `3` (`1` and `2` remain accepted for existing compiled consumers)
...
-   Requested revision: `4` for the VR transition-profile no-op query. Revisions `1`, `2`, and `3` remain accepted for existing binaries, and `0` requests "latest". The bundled `GetCSInterface001()` helper requests revision `4` and returns `nullptr` on older providers so new callers do not accidentally use revision-4 methods on an older vtable.
...
-   `bool IsVRUpscalingTransitionProfileCurrentForMethod(UpscaleMethod method, bool renderScaleModeEnabled, UpscalePreset preset, DLSSProfile profile)`
...
-   `SetVRUpscalingTransitionProfileForMethod` is the preferred revision-2 call for interior/exterior controllers that need deterministic DLSS/FSR behavior. It stages method, Render Scale Mode, shared preset, and DLSS profile together, so `DLSS + NativeAA + K` is unambiguously DLAA/K and `FSR + Hoshipa` is unambiguously FSR render scale. When active unconditional VR FPS Stabilizer Interior/Exterior profiles are available, the two atomic transition-profile calls accept only the configured profile for the opposite cell type. This preserves the documented pre-move call timing while ignoring current-cell profile reassertions during ordinary gameplay. Individual setters and consumers without active stabilizer profiles retain their existing behavior.
-   Revision 4 transition controllers must call `IsVRUpscalingTransitionProfileCurrentForMethod` **before** starting a controller-owned fade. Pass the complete method-specific target profile. `true` means CS has either fully converged to that exact logical and physical contract, or has proved that same destination contract during the CS-owned `LoadingMenu` handoff. Consume that request without starting a fade or calling a setter. The query fails closed for invalid profiles, unknown destinations, unsettled resources, device loss, and incomplete contracts; `false` therefore means the normal protected apply path is required.
-   When the revision-4 no-op query returns `false`, external VR transition controllers should then call `GetVRUpscalingApplyBlockReasons()` or `IsVRUpscalingProfileApplyAllowed()` before applying the profile. A non-zero reason mask means the caller should buffer its latest desired profile, wait, re-check the current destination state and the revision-4 query, then apply once when CS reports that profile application is allowed. Only then should the controller begin its normal fade and call `SetVRUpscalingTransitionProfileForMethod`. Community Shaders also defensively ignores external upscaling API setters while the mask is non-zero.
-   This ordering is required for VR FPS Stabilizer-style controllers: decide whether the request is already current before scheduling `Game.FadeOutGame` or an equivalent fade. Existing Stabilizer releases that begin their fade unconditionally remain ABI-compatible, but must adopt revision 4/build 10 and this call order to eliminate redundant post-load and already-matched-profile fades.
...
-   `IsVRUpscalingTransitionProfileCurrentForMethod` requires interface revision `4` and `getBuildNumber() >= 10`. A controller that cannot obtain revision 4 must retain the revision-3 protected apply/fade behavior and must not call the appended method.
