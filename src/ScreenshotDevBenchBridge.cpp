...
namespace ScreenshotDevBenchBridge
{
	void Install()
	{
		...
		if (g_installAttempted.exchange(true, std::memory_order_acq_rel))
			return;
		static constexpr const char* descriptor = R"({"description":"Versioned asynchronous CSX screenshot and frame-sequence API. Every request uses contractMajor 1 plus clientId and commandId. Capture acceptance returns a stable requestId; request_get and events_poll communicate source, queue, encoding, artifact, sequence, and terminal outcomes. status is read-only. When NR captureFrameEvidence is enabled, accepted stereo acquisitions pin exact CPU exposure and private-measurement companions. Terminal receipts and sequence children contain immutable actual.captureDiagnostics (schemaVersion 1), with transactionId, exposures, measurementBatches and explicit unavailable reasons in measurementRequests. Retention is bounded to 32 keys per publisher, releases at terminal, performs no GPU waits and never changes frozen acquisition evidence. useSettings expands still captures from the CSX screenshot eye/format settings and sequences from the CSX frame eye/format/count/cadence settings.","inputSchema":{"type":"object","required":["contractMajor","action","clientId","commandId"],"properties":{"contractMajor":{"type":"integer","const":1},"contractMinor":{"type":"integer","minimum":0},"action":{"type":"string","enum":["capabilities","status","settings_get","settings_validate","settings_apply","capture","sequence_start","sequence_stop","request_get","request_list","request_cancel","events_poll","acknowledge"]},"clientId":{"type":"string","minLength":1,"maxLength":128},"commandId":{"type":"string","minLength":1,"maxLength":128},"requestId":{"type":"string"},"useSettings":{"type":"boolean","description":"For capture, expand current CSX screenshot settings. For sequence_start, set sequence.useSettings to expand current CSX frame-capture settings."},"capture":{"type":"object"},"sequence":{"type":"object"},"patch":{"type":"object"},"scope":{"type":"string","enum":["runtime_session","persistent_user"]},"afterEventId":{"type":"integer","minimum":0},"throughEventId":{"type":"integer","minimum":0},"limit":{"type":"integer","minimum":1}}},"outputSchema":{"type":"object","properties":{"result":{"type":"object","properties":{"actual":{"type":"object","properties":{"captureDiagnostics":{"type":"object","properties":{"schemaVersion":{"const":1},"finalized":{"const":true},"transactionId":{"type":"string"},"exposures":{"type":"array","maxItems":9},"measurementBatches":{"type":"array","maxItems":8},"measurementRequests":{"type":"array","maxItems":8},"reason":{"type":"string"}}}}}}}}}})";
