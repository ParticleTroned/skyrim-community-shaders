...
namespace SIE
{
	namespace
	{
			...
			const std::filesystem::path& a_shadersRoot)
		{
			...
			auto digest = GetShaderContentDigestInternal(
				a_path,
				a_shadersRoot,
				g_shaderIncludeParseCache,
				g_shaderIncludeParseCacheMutex,
			...
			{
				std::lock_guard lock(g_shaderClosureCacheMutex);
				g_shaderClosureCache.insert_or_assign(cacheKey, ShaderClosureCacheEntry{ generation, digest, {} });
			}
			return digest;
		}
		...
		std::vector<std::string> GetApplicableShaderFeatures(RE::BSShader::Type a_shaderType)
		{
			std::vector<std::string> features;
			for (auto* feature : Feature::GetFeatureList()) {
				if (feature->loaded && feature->HasShaderDefine(a_shaderType))
					features.push_back(feature->GetShortName());
			}
		...
		void InitializeManagedPacks()
		{
			...
			std::call_once(packs.initialized, [&] {
				...
				std::error_code error;
				packs.layoutInstalled = std::ranges::all_of(packPaths, [&](const auto* a_path) {
					error.clear();
					return std::filesystem::is_regular_file(a_path, error) && !error;
				});
				if (!packs.layoutInstalled)
					return;

				try {
					std::ifstream manifestStream(L"Data/ShaderCache/PackManifest.json");
					if (!manifestStream) {
					...
					bool optimizedOpen = packs.optimized->Open(&optimizedError);
					if (optimizedOpen) {
						const auto stats = packs.optimized->GetStats();
						const auto expectedCount = manifest.value("optimizedRecordCount", std::uint64_t{});
						if (stats.activeGeneration == 1 && stats.recordCount < expectedCount) {
							optimizedOpen = false;
							optimizedError = "optimized pack contains fewer records than its manifest contract";
						}
					}
					packs.optimizedAvailable.store(optimizedOpen, std::memory_order_release);
					if (!optimizedOpen)
						logger::warn("Optimized managed shader pack unavailable: {}", optimizedError);

					std::string developerError;
					bool developerOpen = packs.developer->Open(&developerError);
					if (developerOpen) {
						const auto stats = packs.developer->GetStats();
						const auto expectedCount = manifest.value("developerRecordCount", std::uint64_t{});
						if (stats.activeGeneration == 1 && stats.recordCount < expectedCount) {
							developerOpen = false;
							developerError = "developer pack contains fewer records than its manifest contract";
						}
					}
					packs.developerAvailable.store(developerOpen, std::memory_order_release);
					if (!developerOpen)
						logger::warn("Developer managed shader pack unavailable: {}", developerError);
			...
			std::span<const std::string> a_features)
		{
			...
			const auto compatibility = CSX::Api::GetShaderCompatibilityRequirementSet(
				GetShaderPackFamily(a_diskPath), a_shaderPath.string(), a_features);
		...
		{
			try {
				const auto identity = BuildShaderPackIdentity(a_diskPath, a_shaderPath, a_compileStateDigest, a_features);
				if (!identity)
					return nullptr;
				std::string error;
			...
			std::span<const std::string> a_features)
		{
			...
			if (GetShaderPackStore(a_developerMode)) {
				if (!SaveShaderBlobToPack(a_shaderBlob, a_developerMode, a_diskPath, a_shaderPath, a_packCompileStateDigest, a_features))
					return false;
				return true;
			}
	...
	namespace SShaderCache
	{
		...
		*/
		...
		static ID3DBlob* CompileShader(ShaderClass shaderClass, const RE::BSShader& shader, uint32_t descriptor, bool useDiskCache, ShaderFileDependencyTracker* dependencyTracker)
		{
			...
			const auto compatibilityFeatures = GetApplicableShaderFeatures(shader.shaderType.get());
			const auto compatibility = CSX::Api::GetShaderCompatibilityRequirementSet(
			...
			const auto claim = cache.ClaimCompilation(key);
			if (claim.result == ShaderCache::ClaimResult::CacheHit) {
				cache.IncCacheHitTasks();
				return claim.blob;
			}
			const SKSE::stl::scope_exit resolveClaim([&]() noexcept {
				cache.ResolvePendingFailure(key, claim.generation);
			});
			...
			if (managedPack) {
				...
				if (shaderBlob) {
					...
					cache.AddCompletedShader(
						shaderClass, shader, descriptor, shaderBlob, diskPath, compileState.digest, claim.generation, /*fromDisk=*/true);
					return shaderBlob;
				}
		...
		bool fromDisk)
	{
		...
		{
			std::unique_lock lockM{ mapMutex };
			const auto pending = shaderMap.find(key);
			if (pending == shaderMap.end() ||
				pending->second.status != ShaderCompilationTask::Status::Pending ||
				pending->second.claimGeneration != a_claimGeneration) {
				logger::debug("Discarded stale shader completion after cache invalidation: {}", keyWithDescriptor);
				lockM.unlock();
				mapCV.notify_all();
				return false;
			}
			pending->second = ShaderCacheResult{
				a_blob, status, system_clock::now(), fromDisk, a_claimGeneration
			};
		}
		...
		std::string lowerFilePath = Util::FixFilePath(pathString);
		{
			...
			hlslRecord newRecord{
				...
				GetApplicableShaderFeatures(shader.shaderType.get()),
	...
	ShaderCache::CompilationClaim ShaderCache::ClaimCompilation(const std::string& key)
	{
		std::unique_lock lockM{ mapMutex };
			...
			if (it != shaderMap.end()) {
				...
				if (entry.status == ShaderCompilationTask::Status::Completed) {
					if (entry.blob) {
						...
						return { ClaimResult::CacheHit, entry.blob, entry.claimGeneration };
					}
		...
		if (nextCompilationClaimGeneration == 0)
			nextCompilationClaimGeneration = 1;
		const auto generation = nextCompilationClaimGeneration++;
		shaderMap.insert_or_assign(key, ShaderCacheResult{
											nullptr, ShaderCompilationTask::Status::Pending, system_clock::now(), false, generation });
		return { ClaimResult::Claimed, nullptr, generation };
	}
		...
		std::uint64_t a_claimGeneration)
	{
		bool changed = false;
		{
			...
			if (it != shaderMap.end() &&
				...
				it->second.claimGeneration == a_claimGeneration) {
				it->second = ShaderCacheResult{
					nullptr,
					ShaderCompilationTask::Status::Failed,
					system_clock::now(),
					false,
					it->second.claimGeneration
				};
				changed = true;
			}
		}
		...
		std::span<const std::string> a_compatibilityFeatures)
	{
		...
		deferredWrite.compatibilityFeatures.assign(
			a_compatibilityFeatures.begin(), a_compatibilityFeatures.end());
	...
	void ShaderCache::ProcessDeferredDiskWrites(std::stop_token stoken)
	{
		...
		for (;;) {
			...
			if (!writes.empty()) {
				// Requeued above while preserving the global queue-plus-in-flight bound.
			} else if (flushManifest || durableWrites != 0) {
				if (saveLoadDiskPersistenceBlocked.load(std::memory_order_acquire)) {
					std::lock_guard lock{ deferredDiskWritesMutex };
