...
try {
    ...
    if ($onWindows) {
        ...
        if (-not $localSshCommand) {
            ...
            if (-not $sshCommand) {
                $primaryRepositoryRoot = Split-Path -Parent $commonGitDirectory
                $workspaceRoot = Split-Path -Parent $primaryRepositoryRoot
                $authRoot = Join-Path $workspaceRoot "skyrim-vr-automation\artifacts\github-auth"
                $privateKey = Join-Path $authRoot "codex_github_ed25519"
                $knownHosts = Join-Path $authRoot "known_hosts"
                $gitExecutable = Get-Command git.exe -ErrorAction SilentlyContinue
                $gitInstallRoot = if ($gitExecutable) {
                    Split-Path -Parent (Split-Path -Parent $gitExecutable.Source)
                }
                $sshExecutable = if ($gitInstallRoot) {
                    Join-Path $gitInstallRoot "usr\bin\ssh.exe"
                }

                if (
                    $sshExecutable -and
                    (Test-Path -LiteralPath $sshExecutable -PathType Leaf) -and
                    (Test-Path -LiteralPath $privateKey -PathType Leaf) -and
                    (Test-Path -LiteralPath $knownHosts -PathType Leaf)
                ) {
                    $sshCommand = '"{0}" -i "{1}" -o IdentitiesOnly=yes -o UserKnownHostsFile="{2}" -o StrictHostKeyChecking=yes' -f `
                        (ConvertTo-GitPath $sshExecutable),
                        (ConvertTo-GitPath $privateKey),
                        (ConvertTo-GitPath $knownHosts)
                }
            }
