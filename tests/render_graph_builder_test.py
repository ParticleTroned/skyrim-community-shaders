...
def envelope(sequence: int, event_type: str, payload: dict) -> dict:
    return {
        "schema": {"name": "csx.render-event", "major": 1, "minor": 9, "producerVersion": "test"},
...
def normalize_events(events: list[dict]) -> list[dict]:
    for sequence, event in enumerate(events):
        ...
        event["timestampQpc"] = 1000 + sequence
        event["execution"]["commandStreamSequence"] = sequence
    return events
...
def main() -> int:
    ...
    shader_slot_owners = {
        ...
        for path in (repo / "package" / "Shaders").rglob("*")
        if path.is_file() and "register(t127)" in path.read_text(encoding="utf-8", errors="ignore")
    }
    assert shader_slot_owners == {"package/Shaders/Common/CurrentFrameDepthCulling.hlsli"}, shader_slot_owners
    manifest = {
    ...
    hazard_events = [
        ...
        envelope(2, "target-view-observed", {"schema": "target-view-observation-v1", "targetViewObservationId": "obs-shader-resource-view-3-g1", "kind": "shader-resource-view", "d3dObjectPointer": "0x3", "pointerGeneration": 1, "resourceObservationId": "obs-resource-1-g1", "format": 0, "viewDimension": 1, "subresources": {"firstElement": 0, "elementCountOrArraySize": 4}, "flags": 0}),
    ...
    assert graph["schema"]["producerVersion"] == "resource-versions-2"
    ...
    decision_events = [
        ...
        envelope(6, "visibility-candidate", {"schema": "visibility-candidate-v1", "objectIndex": 3, "objectPointer": "0x6", "producerFrame": 1, "engineFrame": 44}),
        ...
        envelope(8, "visibility-result-ready", {"schema": "visibility-result-ready-v1", "resourceVersionObservationId": "obs-resource-version-6-g1", "viewObservationId": "obs-shader-resource-view-3-g1", "objectCount": 4, "producerFrame": 1, "engineFrame": 44}),
        envelope(9, "resource-view-bind", {"schema": "resource-view-binding-v1", "viewObservationId": "obs-shader-resource-view-3-g1", "bindingKind": "shader-resource", "stage": "vertex", "slot": 127}),
        envelope(10, "visibility-consumed", {"schema": "visibility-submission-v1", "submissionObservationId": "obs-submission-7-g1", "renderPassPointer": "0x7", "geometryPointer": "0x8", "objectIndex": 3, "resourceVersionObservationId": "obs-resource-version-6-g1", "requestedViewObservationId": "obs-shader-resource-view-3-g1", "effectiveViewObservationId": "obs-shader-resource-view-3-g1", "category": 1, "slot": 127, "bindingMatches": True, "forcedVisible": False}),
        {**envelope(11, "draw", {"schema": "draw-call-v2", "operation": "draw", "immediateContextPointer": "0x9", "vertexShaderObservationId": None, "pixelShaderObservationId": None, "targetBindingObservationId": "obs-target-binding-5-g1", "submissionObservationId": "obs-submission-7-g1", "arguments": {}}), "submissionObservationId": "obs-submission-7-g1"},
    ...
    decision_events = normalize_events(decision_events)
    ...
    overwrite_events = deepcopy(decision_events)
    draw_index = next(index for index, event in enumerate(overwrite_events) if event["type"] == "draw")
    overwrite_events[draw_index:draw_index] = [
        envelope(0, "target-view-observed", {"schema": "target-view-observation-v1", "targetViewObservationId": "obs-shader-resource-view-overwrite-g1", "kind": "shader-resource-view", "d3dObjectPointer": "0x30", "pointerGeneration": 1, "resourceObservationId": "obs-resource-2-g1", "format": 0, "viewDimension": 1, "subresources": {"firstElement": 0, "elementCountOrArraySize": 4}, "flags": 0}),
        envelope(0, "resource-view-bind", {"schema": "resource-view-binding-v1", "viewObservationId": "obs-shader-resource-view-overwrite-g1", "bindingKind": "shader-resource", "stage": "vertex", "slot": 127}),
    ]
    overwrite_graph = build_graph(tool, manifest, normalize_events(overwrite_events))
    assert overwrite_graph["decisionWindows"][0]["result"] == "not-proven"

    cross_context_events = deepcopy(overwrite_events)
    cross_context_bind = next(
        event for event in cross_context_events
        if event["type"] == "resource-view-bind" and
        event["payload"].get("viewObservationId") == "obs-shader-resource-view-overwrite-g1"
    )
    cross_context_bind["deviceContextObservationId"] = "obs-device-context-2-g1"
    cross_context_graph = build_graph(tool, manifest, normalize_events(cross_context_events))
    assert cross_context_graph["decisionWindows"][0]["result"] == "viable"

    resource_mismatch_events = deepcopy(decision_events)
    version_event = next(event for event in resource_mismatch_events if event["type"] == "resource-version-observed")
    version_event["payload"]["resourceObservationId"] = "obs-resource-2-g1"
    resource_mismatch_graph = build_graph(tool, manifest, normalize_events(resource_mismatch_events))
    assert resource_mismatch_graph["decisionWindows"][0]["result"] == "not-proven"

    repeated_producer_events = deepcopy(decision_events)
    ready_index = next(index for index, event in enumerate(repeated_producer_events) if event["type"] == "visibility-result-ready")
    repeated_producer_events[ready_index + 1:ready_index + 1] = [
        envelope(0, "resource-version-observed", {"schema": "resource-version-observation-v1", "resourceVersionObservationId": "obs-resource-version-repeated-g1", "resourceObservationId": "obs-resource-1-g1", "subresources": {"first": 0, "count": 1}, "writeEpoch": 5, "producerFrame": 1, "readinessDomain": "same-immediate-context-order", "eye": "unknown", "eyeMask": None}),
        envelope(0, "visibility-result-ready", {"schema": "visibility-result-ready-v1", "resourceVersionObservationId": "obs-resource-version-repeated-g1", "viewObservationId": "obs-shader-resource-view-3-g1", "objectCount": 4, "producerFrame": 1, "engineFrame": 44}),
    ]
    repeated_producer_graph = build_graph(tool, manifest, normalize_events(repeated_producer_events))
    assert repeated_producer_graph["decisionWindows"][0]["result"] == "viable"
    assert repeated_producer_graph["decisionWindows"][0]["extensions"]["csx.resourceVersionObservationId"] == "obs-resource-version-6-g1"
