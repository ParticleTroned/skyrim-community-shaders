...
void SkySync::Update(const RE::Sky* sky)
{
	...
	volumetricLightingIntensityFactor = NormalizeVolumetricLightingIntensity(
		std::max(currentDim,
			std::max(intensities[static_cast<int>(Caster::Masser)],
				intensities[static_cast<int>(Caster::Secunda)])));
}
