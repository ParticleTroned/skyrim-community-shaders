...
#include "../../src/Utils/CharacterCategoryAuthoring.h"
...
namespace
{
	...
	public:
		...
		MaskResult Mask(const MaskConstants& constants, std::uint32_t packedWidth,
			...
			std::uint32_t currentDepthWidth = 0, std::uint32_t currentDepthHeight = 0)
		{
			...
			if (currentDepthWidth == 0)
				currentDepthWidth = constants.sourceCrop[3];
			if (currentDepthHeight == 0)
				currentDepthHeight = constants.options[0];
			Require(currentDepthWidth >= constants.sourceCrop[3] && currentDepthHeight >= constants.options[0],
				"Current depth allocation must contain the active input");
			if (currentDepth.empty())
				currentDepth.assign(currentDepthWidth * currentDepthHeight, 0.5f);
			Require(authoredDepth.size() == tuples.size(), "Authored depth dimensions");
			Require(currentDepth.size() == currentDepthWidth * currentDepthHeight,
				"Current depth dimensions");
			auto category = MakeTexture(packedWidth, sourceHeight, CharacterCategoryAuthoring::kTargetFormat,
			...
			auto current = MakeTexture(currentDepthWidth, currentDepthHeight,
		...
		void CapturePreservesOutsideAndBothChannels()
		{
			...
			auto sourceTuple = MakeTexture(width, height, CharacterCategoryAuthoring::kTargetFormat,
				D3D11_BIND_SHADER_RESOURCE, tuples);
			...
			auto frozenTuple = MakeTexture(width, height, CharacterCategoryAuthoring::kTargetFormat,
			...
			// Intentionally odd extent at a nonzero source offset exercises rounded dispatch.
		...
		void VertexAoPreservesOriginalPrecision()
		{
			...
			auto categories = MakeTexture(1, 1, CharacterCategoryAuthoring::kTargetFormat,
		...
		void CompositePreservesBaseline(const std::filesystem::path& shaderPath)
		{
			...
			auto blob = Compile(shaderPath);
			ComPtr<ID3D11ShaderReflection> reflection;
			Check(D3DReflect(blob->GetBufferPointer(), blob->GetBufferSize(),
					  __uuidof(ID3D11ShaderReflection), &reflection),
				"Reflect character composite");
			auto* reflectedConstants = reflection->GetConstantBufferByName("CompositeData");
			D3D11_SHADER_BUFFER_DESC constantsDescription{};
			Check(reflectedConstants->GetDesc(&constantsDescription), "Reflect composite constants");
			Require(constantsDescription.Size == sizeof(CompositeConstants), "Composite constant ABI differs");
			D3D11_SHADER_VARIABLE_DESC supportDescription{};
			Check(reflectedConstants->GetVariableByName("MaskSupport")->GetDesc(&supportDescription),
				"Reflect composite support");
			Require(supportDescription.StartOffset == offsetof(CompositeConstants, support),
				"Composite support ABI offset differs");
			ComPtr<ID3D11ComputeShader> shader;
			Check(device_->CreateComputeShader(blob->GetBufferPointer(), blob->GetBufferSize(),
					  nullptr, &shader),
				"Create character composite shader");
			constexpr std::uint32_t width = 9, height = 7;
			const float nan = std::numeric_limits<float>::quiet_NaN();
			std::vector<Color> baseline(width * height), neural(width * height);
			std::vector<std::uint8_t> weights(width * height);
			for (std::uint32_t y = 0; y < height; ++y) {
				for (std::uint32_t x = 0; x < width; ++x) {
					const auto index = y * width + x;
					baseline[index] = { 0.1f + x * 0.03f, 0.2f + y * 0.02f, -0.4f, 0.1f + x * 0.01f };
					neural[index] = { 1.5f, 0.7f, 0.9f, nan };
					weights[index] = std::array<std::uint8_t, 4>{ 0, 64, 128, 255 }[x % 4];
					if (x == 0 || x == width - 1 || y == 0 || y == height - 1)
						neural[index] = { nan, nan, nan, nan };
				}
			}
			neural[3 * width + 3][0] = nan;
			neural[3 * width + 6][1] = std::numeric_limits<float>::infinity();
			auto base = MakeTexture(width, height, DXGI_FORMAT_R32G32B32A32_FLOAT,
				D3D11_BIND_SHADER_RESOURCE, baseline);
			auto nr = MakeTexture(width, height, DXGI_FORMAT_R32G32B32A32_FLOAT,
				D3D11_BIND_SHADER_RESOURCE, neural);
			auto mask = MakeTexture(width, height, DXGI_FORMAT_R8_UNORM,
				D3D11_BIND_SHADER_RESOURCE, weights);
			for (const bool emptySupport : { false, true }) {
				for (std::uint32_t debug = 0; debug < 4; ++debug) {
					CompositeConstants constants{ { width, height }, debug, 0, { 1, 1, 8, 6 } };
					if (emptySupport)
						std::fill_n(constants.support, 4, 0u);
					auto output = MakeTexture(width, height, DXGI_FORMAT_R32G32B32A32_FLOAT,
						D3D11_BIND_UNORDERED_ACCESS, std::vector<Color>(width * height, { -9.0f, -9.0f, -9.0f, -9.0f }));
					auto cb = Constants(constants);
					std::array<ID3D11ShaderResourceView*, 3> inputs{ base.srv.Get(), nr.srv.Get(), mask.srv.Get() };
					...
					context_->Dispatch((width + 7u) / 8u, (height + 7u) / 8u, 1);
					context_->ClearState();
					const auto actual = Read<Color>(output.resource.Get());
					for (std::uint32_t y = 0; y < height; ++y) {
						for (std::uint32_t x = 0; x < width; ++x) {
							const auto index = y * width + x;
							const bool supported = !emptySupport && x >= 1 && x < 8 && y >= 1 && y < 6;
							const float weight = supported ? weights[index] / 255.0f : 0.0f;
							Color expected = baseline[index];
							const bool validNeural = std::isfinite(neural[index][0]) &&
							                         std::isfinite(neural[index][1]) && std::isfinite(neural[index][2]);
							for (std::size_t channel = 0; channel < 3; ++channel) {
								if (validNeural && (weight > 0.0f || debug == 3))
									expected[channel] = debug == 3 ? neural[index][channel] :
									                                 baseline[index][channel] + weight * (neural[index][channel] - baseline[index][channel]);
								if (debug == 1)
									expected[channel] = weight;
								if (debug == 2 && supported && (x < 3 || y < 3 || x >= 6 || y >= 4))
									expected[channel] = channel == 1 ? 1.0f : 0.0f;
								Require(std::isfinite(actual[index][channel]) &&
											std::abs(actual[index][channel] - expected[channel]) < 0.000001f,
									"Character composite RGB, support, debug mode, or nonfinite fallback differs");
							}
							Require(actual[index][3] == baseline[index][3], "Character composite modified baseline alpha");
							if (debug == 0 && (!supported || weight == 0.0f || !validNeural))
								Require(actual[index] == baseline[index], "Character composite modified an untouched baseline pixel");
						}
					}
				}
			}
	...
	private:
		...
		ComPtr<ID3D11ComputeShader> maskShader_, captureShader_;
	};
	...
	void CropsAndDirtyRegions(Harness& gpu)
	{
		...
		constants.outputAndSourceSize[2] = 8;
		constants.sourceCrop[1] = 4;
		constants.authoredRegion[2] = 8;
		constants.categoryStrengths[0] = 0.25f;
		constants.categoryStrengths[1] = 0.75f;
		std::vector<Tuple> source(16);
		for (std::uint32_t y = 0; y < 2; ++y) {
			for (std::uint32_t x = 0; x < 8; ++x)
				source[y * 8 + x] = { 0, static_cast<std::uint8_t>(x < 4 ? 85 : 170) };
		}
		const auto cropped = gpu.Mask(constants, 8, source);
		for (auto value : cropped.pixels)
			ExpectByte(value, 191, "Cropped input uses its original source coordinates");
		constants.sourceCrop[1] = 0;
		const auto uncropped = gpu.Mask(constants, 8, source);
		for (auto value : uncropped.pixels)
			ExpectByte(value, 64, "Moving crop follows the selected material");
	}
}
...
{
	try {
		Require(argc == 2 || argc == 3, "Expected production shader directory and optional composite shader path");
		Harness gpu(argv[1]);
		...
		CropsAndDirtyRegions(gpu);
		CurrentDepthAllowsLargerAllocation(gpu);
		if (argc == 3)
			gpu.CompositePreservesBaseline(argv[2]);
		std::cout << "Production character capture/mask/composite HLSL passed WARP synthetic tests\n";
		return 0;
