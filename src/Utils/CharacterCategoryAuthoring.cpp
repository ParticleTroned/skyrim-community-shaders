...
void CharacterCategoryAuthoring::Capture()
{
	const auto& upscaling = globals::features::upscaling;
	auto* state = globals::state;
	auto* renderer = globals::game::renderer;
	if (!state || !state->inWorld || !globals::deferred ||
		!globals::deferred->deferredPass || !renderer ||
	...
	if (!upscaling.GetCharacterNeuralRenderingProjectionExtent(outputWidth, outputHeight))
		return;
	const auto renderSize = Util::ConvertToDynamic(
		float2{ static_cast<float>(outputWidth), static_cast<float>(outputHeight) });
	if (!std::isfinite(renderSize.x) || !std::isfinite(renderSize.y) ||
		renderSize.x < 1.0f || renderSize.y < 1.0f ||
		renderSize.x > D3D11_REQ_TEXTURE2D_U_OR_V_DIMENSION ||
		renderSize.y > D3D11_REQ_TEXTURE2D_U_OR_V_DIMENSION)
		return;

	auto& categories = renderer->GetRuntimeData().renderTargets[MASKS2];
	auto& depth = renderer->GetDepthStencilData()
	                  .depthStencils[RE::RENDER_TARGETS_DEPTHSTENCIL::kMAIN];
	(void)NeuralRendering::CharacterRendering::Instance().CaptureAuthoredCategories(
		globals::d3d::device, globals::d3d::context,
		categories.texture, depth.depthSRV,
		static_cast<uint32_t>(renderSize.x), static_cast<uint32_t>(renderSize.y),
		state->frameCount, upscaling.GetCharacterNeuralRenderingCategoryMask(),
