...
namespace
{
	...
	template <class T, std::size_t N> T Parse(const Json& value, const std::array<const char*, N>& names)
	{
		const auto text = value.get<std::string>();
		for (std::size_t i = 0; i < names.size(); ++i) if (text == names[i]) return static_cast<T>(i);
		throw std::invalid_argument("unknown enum value: " + text);
	}
	template <class T, std::size_t N> const char* Name(T value, const std::array<const char*, N>& names)
	{
		const auto index = static_cast<std::size_t>(value); return index < names.size() ? names[index] : "invalid";
	}
	void Keys(const Json& object, std::initializer_list<std::string_view> allowed)
	{
		...
		for (const auto& [key, value] : object.items()) {
			(void)value;
			if (std::find(allowed.begin(), allowed.end(), key) == allowed.end()) throw std::invalid_argument("unknown field: " + key);
		}
	}
	...
	void ReadSettings(const Json& object, Settings& settings)
	{
		...
		if (object.contains("enabled")) settings.enabled = object.at("enabled").get<bool>();
	...
	void ReadProfile(const Json& object, Profile& profile)
	{
		...
		if (object.contains("exposureSource")) profile.exposureSource = Parse<ExposureSource>(object.at("exposureSource"), exposureSources);
		if (!Valid(profile)) throw std::invalid_argument("codec requires explicit linear source; identity requires manual multiplier one");
	}
	...
	Json CaptureJson()
	{
		...
		for (const auto& e : capture.samples) if (e.stamp.sequence) samples.push_back(EvidenceJson(e));
	...
	Json StatusJson()
	{
		const auto config = Registry::Instance().Snapshot(); const auto status = Registry::Instance().GetStatus();
		Json slots = Json::array(), measurements = Json::array();
		for (const auto& o : status.slots) if (o.revision) slots.push_back(ObservationJson(o));
		for (const auto& m : status.measurements) {
			if (!m.source.revision) continue;
	...
	Json AssetsJson()
	{
		...
		Json assets = Json::array(); bool complete = true;
		for (const auto* path : paths) {
			std::error_code error; const bool present = std::filesystem::is_regular_file(path, error);
	...
	{
		if (!write) return;
		Json response; std::string errorCode = "nr_color_invalid_request";
		try {
			const auto request = Json::parse(arguments ? arguments : "{}"); const auto action = request.at("action").get<std::string>();
			if (action == "status" || action == "assets") {
				...
				if (request.contains("expectedRevision")) {
					...
					if (!expected.is_number_integer() || expected <= 0) throw std::invalid_argument("invalid revision");
					if (expected.get<std::uint64_t>() != config.revision) { errorCode = "nr_color_revision_conflict"; throw std::invalid_argument("configuration changed; read status again"); }
				}
				if (action == "reset_experiments") {
					if (request.contains("settings") || request.contains("experiments")) throw std::invalid_argument("reset_experiments does not accept settings");
					config.experiments = {};
				} else {
					if (!request.contains("settings") && !request.contains("experiments")) throw std::invalid_argument("configure requires settings or experiments");
					...
					if (request.contains("experiments")) {
						...
						if (e.contains("captureEngineExposure")) config.experiments.captureEngineExposure = e.at("captureEngineExposure").get<bool>();
						if (e.contains("applyModelEdit")) config.experiments.applyModelEdit = e.at("applyModelEdit").get<bool>();
					}
				}
				if (!Registry::Instance().Configure(config.settings, config.experiments, config.revision)) {
					errorCode = "nr_color_revision_conflict"; throw std::invalid_argument("invalid or concurrently changed configuration");
				}
			} else throw std::invalid_argument("unknown action");
			response = action == "assets" ? AssetsJson() : StatusJson(); response["action"] = action;
		} catch (const std::exception& error) { response = { { "ok", false }, { "error", error.what() }, { "errorCode", errorCode } }; }
		catch (...) { response = { { "ok", false }, { "error", "colour handler failed" }, { "errorCode", "nr_color_handler_failed" } }; }
		try { const auto text = response.dump(); write(sink, text.c_str()); }
		catch (...) { write(sink, R"({"ok":false,"error":"colour status serialization failed"})"); }
	}
	Json Descriptor()
	{
...
NeuralColor& NeuralColor::Instance() { static NeuralColor instance; return instance; }
void NeuralColor::LoadSettings(nlohmann::json& object)
{
	Settings settings;
	try { ReadSettings(object, settings); }
	catch (const std::exception& error) { logger::warn("[NRColor] Invalid saved settings; using Legacy Raw: {}", error.what()); settings = {}; }
...
{
	auto config = Registry::Instance().Snapshot(); bool changed = false;
	...
	if (ImGui::TreeNode("Exposure capture and colour assessment")) {
		...
		for (std::size_t i = 0; i < config.experiments.profiles.size(); ++i) {
			ImGui::PushID(static_cast<int>(i)); ImGui::Separator(); ImGui::TextUnformatted(i ? "Final LDR pre-UI" : "Upscaled Centre");
			auto& p = config.experiments.profiles[i]; int domain = static_cast<int>(p.domain), transform = static_cast<int>(p.transform);
			changed |= ImGui::Combo("Source-domain candidate", &domain, "Unknown/native\0Linear RGB\0sRGB encoded\0"); p.domain = static_cast<Domain>(domain);
			if (p.domain != Domain::Linear && p.transform != Transform::Identity) { p.transform = Transform::Identity; changed = true; }
			if (p.domain == Domain::Linear) {
				transform = static_cast<int>(p.transform);
				changed |= ImGui::Combo("Model input transform", &transform, "Identity\0Linear to sRGB\0Reversible proxy + sRGB\0"); p.transform = static_cast<Transform>(transform);
			}
			if (p.transform == Transform::Identity) { p.exposureMultiplier = 1; p.exposureSource = ExposureSource::Manual; }
			else {
				int source = static_cast<int>(p.exposureSource);
				changed |= ImGui::Combo("Exposure source", &source, "Manual calibration\0Captured engine HDR (frame matched)\0"); p.exposureSource = static_cast<ExposureSource>(source);
				float stops = std::log2(p.exposureMultiplier);
				if (ImGui::SliderFloat("Calibration multiplier (EV)", &stops, -8, 8)) { p.exposureMultiplier = std::exp2(stops); changed = true; }
			}
			ImGui::PopID();
		}
		...
		if (ImGui::Button("Reset assessment overrides")) { config.experiments = {}; changed = true; }
		ImGui::TreePop();
	}
	...
	if (ImGui::TreeNode("Colour diagnostics")) {
		const auto text = StatusJson().dump(2); ImGui::TextUnformatted(text.c_str()); ImGui::TreePop();
	}
	static std::string assets;
	if (ImGui::Button("Check installed colour shaders")) assets = AssetsJson().dump(2);
	if (!assets.empty()) ImGui::TextUnformatted(assets.c_str());
}
