...
#define CSX_DEPTH_CULLING_SNAPSHOT_FIELDS(X)             \
	...
	X(std::uint64_t, renderPassMismatch, 0)              \
	...
	X(std::uint64_t, invalidVisibilityContract, 0)       \
	X(std::uint64_t, descriptorMismatch, 0)              \
	X(std::uint64_t, bindingRejected, 0)                 \
...
#undef CSX_DECLARE_SNAPSHOT_FIELD
	...
	public:
		...
		{
			std::scoped_lock lock(resetMutex);
			const auto resume = collecting.load(std::memory_order_acquire);
			QuiesceAndReset(resume);
		}
...
#	define CSX_FAIL_OPEN_RECORD(method, field, reason)                                         \
		...
		CSX_FAIL_OPEN_RECORD(RenderPassMismatch, renderPassMismatch, RenderPassMismatch)
		...
		CSX_FAIL_OPEN_RECORD(InvalidVisibilityContract, invalidVisibilityContract, InvalidVisibilityContract)
		CSX_FAIL_OPEN_RECORD(DescriptorMismatch, descriptorMismatch, DescriptorMismatch)
		CSX_FAIL_OPEN_RECORD(BindingRejected, bindingRejected, BindingRejected)
...
#	undef CSX_SIMPLE_RECORD
		...
		{
			MutationGuard guard(*this);
			if (!guard || a_epoch != CollectionEpoch())
				return;
		...
		{
			for (;;) {
				const auto sequenceBefore = resetSequence.load(std::memory_order_acquire);
				if (sequenceBefore & 1) {
...
#	undef CSX_CAPTURE_FIELD
				...
				if (resetSequence.load(std::memory_order_acquire) == sequenceBefore)
					return result;
			}
		}
	...
	private:
		...
		{
		public:
			explicit MutationGuard(Counters& a_owner) : owner(&a_owner)
			{
				if (!owner->collecting.load(std::memory_order_acquire))
					return;
				owner->activeWriters.fetch_add(1, std::memory_order_acq_rel);
				if (owner->collecting.load(std::memory_order_acquire)) {
					active = true;
				} else {
					owner->activeWriters.fetch_sub(1, std::memory_order_release);
				}
			}
		...
		private:
			Counters* owner = nullptr;
			bool active = false;
		};

		void QuiesceAndReset(bool a_resume)
		{
			BeginQuiesce();
		...
		void EndQuiesce(bool a_resume)
		{
			collecting.store(a_resume, std::memory_order_release);
			resetSequence.fetch_add(1, std::memory_order_release);
		}
...
#	undef CSX_RESET_FIELD
		...
		std::atomic_uint64_t activeWriters{ 0 };
		std::atomic_uint64_t resetSequence{ 0 };
...
#	define CSX_STUB_FAIL_OPEN(name) \
		void Record##name(DrawCategory = DrawCategory::Lighting) {}
		...
		CSX_STUB_FAIL_OPEN(RenderPassMismatch)
		...
		CSX_STUB_FAIL_OPEN(InvalidVisibilityContract)
		CSX_STUB_FAIL_OPEN(DescriptorMismatch)
		CSX_STUB_FAIL_OPEN(BindingRejected)
		CSX_STUB_FAIL_OPEN(ForcedVisibleSrvUnavailable)
#	undef CSX_STUB_FAIL_OPEN
#	define CSX_STUB_RECORD(name) \
		void Record##name() {}
