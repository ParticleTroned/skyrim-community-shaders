...
PS_OUTPUT main(PS_INPUT input, bool frontFace : SV_IsFrontFace)
{
	...
	baseColor.xyz = pow(abs(baseColor.xyz),
SharedData::lodBlendingSettings.LODObjectGamma) * 
SharedData::lodBlendingSettings.LODObjectBrightness;
...
#endif
	baseColor.xyz = pow(abs(baseColor.xyz),
SharedData::lodBlendingSettings.LODTerrainGamma) * 
SharedData::lodBlendingSettings.LODTerrainBrightness;
#endif
#endif  // LOD_BLENDING
...
#if defined(LOD_BLENDING)
	lodLandColor.xyz = pow(abs(lodLandColor.xyz), 
SharedData::lodBlendingSettings.LODTerrainGamma) * 
SharedData::lodBlendingSettings.LODTerrainBrightness;
#endif  // LOD_BLENDING
	...
	if (ProjectedUVParams3.w > 0.5) {
		...
		projBaseColor.xyz = pow(abs(projBaseColor.xyz), 
	SharedData::lodBlendingSettings.LODObjectSnowGamma) * 
	SharedData::lodBlendingSettings.LODObjectSnowBrightness;
#endif  // TRUE_PBR
...
if (alpha - AlphaTestRefRS < 0) {
	...
	float sssMask = 0.0;
#if defined(SKIN)
	// Skin: existing behavior (SSS amount from alpha)
	sssMask = saturate(baseColor.a);
#elif defined(HAIR)
	// Brows / hairline overlays (alpha blended): write a *tight edge-band* SSS mask so only the transition to skin is softened.
	// This avoids mask/noise patterns from affecting full hair/beard interiors.
	float a = saturate(baseColor.a);
	float strength = saturate(SharedData::SSSBrowMaskBlendStrength);

	if (strength > 0.0) {
		// Edge detector: only triggers where alpha changes (hairline/brow boundary).
		float edge = saturate(fwidth(a) * SharedData::SSSBrowMaskEdgeSensitivity);

		// Alpha band window: keep the effect near the transition (not fully transparent, not mostly opaque).
		float band = smoothstep(0.02, 0.15, a) * (1.0 - smoothstep(0.35, 0.70, a));

		sssMask = strength * edge * band;

		// Optional depth gate (adds one depth read): further restrict to overlays sitting very close to underlying skin.
		if (SharedData::SSSBrowMaskDepthGateEnabled != 0) {
			float depthScene = SharedData::DepthTexture.Load(int3(int2(input.Position.xy), 0)).x;
			float depthSceneLinear = SharedData::GetScreenDepth(depthScene);
			float depthPixelLinear = SharedData::GetScreenDepth(input.Position.z);
			float dz = abs(depthSceneLinear - depthPixelLinear);

			float thresh = max(SharedData::SSSBrowMaskDepthThreshold, 1e-6);
			float gate = saturate(1.0 - dz / thresh);
			sssMask *= gate;
		}
	} else {
		sssMask = 0.0;
