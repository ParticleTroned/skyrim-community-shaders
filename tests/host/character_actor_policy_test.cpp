...
int main()
{
	...
	if (ResolveCharacterProjection(corners, 100, 200, rect) != CharacterProjectionResult::Offscreen || rect.IsValid())
		return 2;
	// An offscreen bound remains empty after an independent visible projection.
	CharacterRect independentProjection{};
	if (ResolveCharacterProjection(visibleCorners, 100, 200, independentProjection) != CharacterProjectionResult::Visible ||
		rect.IsValid())
		return 3;
