...
{
	++checks;
	if (!value) { std::fprintf(stderr, "%s (check %u)\n", message, checks); std::abort(); }
}
...
int main(int argc, char** argv)
{
	...
	D3D11_BUFFER_DESC cbDesc{}; cbDesc.ByteWidth = sizeof(Constants); cbDesc.Usage = D3D11_USAGE_DEFAULT;
	...
	for (unsigned transform = 0; transform < 3; ++transform) {
		...
		for (unsigned insertion = 0; insertion < 2; ++insertion) {
			...
			constants.x = insertion ? 30u : 5u; constants.y = insertion ? 40u : 7u;
	...
	for (unsigned y = 0; y < constants.height; ++y) for (unsigned x = 0; x < constants.width; ++x) {
		auto value = original[y * 64 + x]; value.r *= 2; value.g *= 2; value.b *= 2; value.a = 0.0f;
