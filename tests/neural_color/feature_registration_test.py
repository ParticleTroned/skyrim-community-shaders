...
MANIFEST = Path("features/Neural Rendering/Shaders/Features/NeuralRendering.ini")
CORE = Path("features/Neural Rendering/CORE")
...
class RegistrationTests(unittest.TestCase):
    ...
    def test_generated_header_cannot_mask_missing_runtime_entry(self):
        ...
        registry.write_text("\n".join(line for line in text.splitlines()
                                      if not ('"NeuralRendering"sv' in line and '{' in line)))
        result = self.validate()
    ...
    def test_missing_core_registration(self):
        ...
        text = registry.read_text()
        registry.write_text("\n".join(line for line in text.splitlines()
                                      if line.strip() != '"NeuralRendering"sv,'))
        result = self.validate()
        self.assertNotEqual(result.returncode, 0)
