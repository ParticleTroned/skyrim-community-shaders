...
namespace
{
	...
	const std::array<DofAutoFocusSettingDefinition, 7> kDofAutoFocusSettingDefinitions{ {
		{ "fDynamicDOFNearDist:Display", &DofAutoFocusSettings::nearDistance, 0.0f, kDofDistanceMin, kDofAutoFocusDepthMax, REL::VariantOffset(0x323369c, 0x338cacc, 0) },
		{ "fDynamicDOFFarDist:Display", &DofAutoFocusSettings::farDistance, 0.0f, kDofDistanceMin, kDofAutoFocusDepthMax, REL::VariantOffset(0x32336a0, 0x338cad0, 0) },
		{ "fDynamicDOFNearRange:Display", &DofAutoFocusSettings::nearRange, 0.0f, kDofRangeMin, kDofAutoFocusDepthMax, REL::VariantOffset(0x32336a4, 0x338cad4, 0) },
		{ "fDynamicDOFFarRange:Display", &DofAutoFocusSettings::farRange, 0.0f, kDofRangeMin, kDofAutoFocusDepthMax, REL::VariantOffset(0x32336a8, 0x338cad8, 0) },
		{ "fDynamicDOFNearBlur:Display", &DofAutoFocusSettings::nearBlur, 0.0f, kDofAutoFocusBlurMin, kDofAutoFocusBlurMax, REL::VariantOffset(0x32336ac, 0x338cadc, 0) },
		{ "fDynamicDOFFarBlur:Display", &DofAutoFocusSettings::farBlur, 0.0f, kDofAutoFocusBlurMin, kDofAutoFocusBlurMax, REL::VariantOffset(0x32336b0, 0x338cae0, 0) },
		{ "fDynamicDOFBlurMultiplier:Display", &DofAutoFocusSettings::blurMultiplier, 1.0f, kDofAutoFocusBlurMultiplierMin, kDofAutoFocusBlurMultiplierMax, REL::VariantOffset(0x32336b4, 0x338cae4, 0) },
	} };
...
{
	ImGui::SeparatorText(T(TKEY("tab_vanilla_dof"), "Vanilla Depth of Field"));
	static Util::ConfirmationPopup sceneLockPopup;
	static Util::ConfirmationPopup underwaterLockPopup;

	ImGui::Checkbox(
		T(TKEY("underwater_fog_blur_fix"), "Enable Underwater Fog DOF Blur Fix"),
		&settings.fixUnderwaterFogDofBlur);
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::TextUnformatted(T(
			TKEY("underwater_fog_blur_fix_tooltip"),
			"Correctly blurs underwater fog with vanilla DOF. This affects only the fix and takes effect immediately."));
	}
	ImGui::Separator();

	if (!IsRuntimeEnabled()) {
		Util::TextUnformattedDisabled(T(
			TKEY("dof_disabled"),
			"CS Utility is disabled. Saved DOF settings and the fog-blur fix are not applied."));
		ImGui::Separator();
	}
