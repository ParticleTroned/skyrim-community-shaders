...
// Resolves the frozen active-frame provenance tuple into one exact view-local
...
{
	uint4 OutputAndSourceSize;                                // output width/height, source width/height
	uint4 SourceCrop;                                         // reserved zero, input min X/Y, input width
	...
	row_major float4x4 CameraProjInverse;                     // current view projection inverse
	float4 Jitter;                                            // current low-resolution pixel jitter in xy
	float4 CategoryStrengths;                                 // face, skin, hair, reserved
	float4 EligibilityRectangles[EligibilityRegionCapacity];  // view-local output pixels, min.xy/max.xy
	uint4 DispatchRegion;                                     // output-local offset.xy and dispatch extent.zw
	uint4 AuthoredRegion;                                     // valid current-frame full-view source offset.xy/extent.zw
};
...
uint ReadAuthoredCategory(int2 localSourcePixel)
{
	...
	const int2 viewPixel = sourcePixel - int2(SourceCrop.x, 0);
	if (any(viewPixel < int2(AuthoredRegion.xy)) ||
		any(viewPixel >= int2(AuthoredRegion.xy + AuthoredRegion.zw)))
		return 0u;
...
{
	const int2 viewPixel = GetGlobalSourcePixel(localSourcePixel) - int2(SourceCrop.x, 0);
	if (any(viewPixel < int2(AuthoredRegion.xy)) ||
		any(viewPixel >= int2(AuthoredRegion.xy + AuthoredRegion.zw)))
		return 1.0;
...
float ReconstructCameraDistance(int2 localSourcePixel, float rawDepth)
{
	const int2 inputSize = int2(SourceCrop.w, Options.x);
	localSourcePixel = clamp(localSourcePixel, int2(0, 0), inputSize - 1);
	const float2 viewPixel =
		float2(int2(SourceCrop.y, SourceCrop.z) + localSourcePixel) + 0.5;
	const float2 uv = viewPixel / float2(OutputAndSourceSize.zw);
...
float GetDistanceWeight(int2 localSourcePixel, float rawDepth)
{
	if (VisibilityOptions.z <= 0.0)
		return 1.0;
	const float cameraDistance = ReconstructCameraDistance(localSourcePixel, rawDepth);
	const float fadeWidth = min(
		VisibilityOptions.z,
		max(VisibilityOptions.w, 1.0e-6));
	return saturate((VisibilityOptions.z - cameraDistance) / fadeWidth);
}
