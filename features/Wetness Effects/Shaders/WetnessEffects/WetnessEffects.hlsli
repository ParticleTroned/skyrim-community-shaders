...
namespace WetnessEffects
{
	...
	float4 GetRainDrops(float3 worldPos, float t, float3 normal, float rippleStrengthModifier = 1.0, float2 flowOffset = float2(0.0, 0.0))
	{
		...
		// Legacy chaotic ripple turbulence (optional)
        if (SharedData::wetnessEffectsSettings.EnableChaoticRipples != 0)
        {
			// noise() returns scalar; assigning to float3 replicates (legacy behavior)
            float3 turbulenceNormal = noise(float3(
				worldPos.xy * SharedData::wetnessEffectsSettings.ChaoticRippleScaleRcp,
				t * SharedData::wetnessEffectsSettings.ChaoticRippleSpeed));

            turbulenceNormal.z = turbulenceNormal.z * 0.5 + 5.0;
            turbulenceNormal = normalize(turbulenceNormal);

            rippleNormal = normalize(rippleNormal + float3(
				turbulenceNormal.xy * SharedData::wetnessEffectsSettings.ChaoticRippleStrength, 0.0));
        }
