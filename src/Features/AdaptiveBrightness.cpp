...
std::optional<AdaptiveBrightness::ContextSection> AdaptiveBrightness::SyncContextSection()
{
	...
	if (activeOverride) {
		...
		return GetCurrentContextOverrideScope(activeOverride) ? ContextSection::Profiles : ContextSection::Locations;
	}
	...
	bool a_allowEdits)
{
	...
	ImGui::SameLine();
	ImGui::BeginDisabled(!a_allowEdits || !a_locationOverride);
	...
	const LocationOverrideTarget& a_target)
{
	...
	try {
		importedProfile = profileJson->get<ProfileSettings>();
	} catch (const json::exception& e) {
