...
float3 GetWetReflectionModeConfig(float wetReflectionScale)
{
	...
	const bool enableModern = CS_WETNESS_SETTINGS.EnableModernWetReflection != 0;
	const bool enableLegacy = CS_WETNESS_SETTINGS.EnableLegacyWetReflection != 0;
...
#	if defined(SKIN) || defined(HAIR)
	const float skinWetnessOverdriveT = saturate((CS_WETNESS_SETTINGS.SkinWetness - 1.0) / 0.5);
...
WetnessDirectLightingParams CreateWetnessDirectLightingParams(float3 wetnessNormal, float3 viewDir, float roughness, float3 wetReflectionModeConfig)
{
	...
	const bool forwardReflectionBiasEnabled = CS_WETNESS_SETTINGS.EnableForwardReflectionBias != 0;
	const bool vanillaReflectionCompensationEnabled = CS_WETNESS_SETTINGS.EnableVanillaReflectionCompensation != 0;
