...
namespace PBR
{
	...
	void GetDirectLightInput(out DirectLightingOutput lightingOutput, DirectContext context, MaterialProperties material, float3x3 tbnTr, float2 uv)
	{
...
#endif
		{
			...
			{
				// Animated foliage has no reliable thickness after wind deformation.
				// Use only the current normal, light, and view angles.
...
#	endif
...
#endif
			const float metalHighlightScale = lerp(1.0, SharedData::truePBRSettings.PBRMetalHighlightScale, saturate(material.Metallic));
	...
	void GetIndirectLobeWeights(out IndirectLobeWeights lobeWeights, IndirectContext context, MaterialProperties material)
	{
...
#if defined(TREE_ANIM)
		// Add a small AO-independent indirect response for animated foliage.
		...
		const float metalReflectionScale = lerp(1.0, SharedData::truePBRSettings.PBRMetalReflectionScale, saturate(material.Metallic));
