...
namespace NeuralRendering::Color
{
	...
	namespace
	{
		...
		bool CreateReadback(ID3D11Device* device, Readback& readback)
		{
			...
			if (FAILED(device->CreateBuffer(&desc, nullptr, &readback.gpu)) ||
				FAILED(device->CreateUnorderedAccessView(readback.gpu.Get(), &view, &readback.uav))) return false;
	...
	Registry& Registry::Instance() { static Registry instance; return instance; }
	Configuration Registry::Snapshot() const { std::scoped_lock lock(mutex_); return configuration_; }
	Status Registry::GetStatus() const { std::scoped_lock lock(mutex_); return status_; }
	bool Registry::Configure(const Settings& settings, const Experiments& experiments, std::uint64_t expectedRevision)
	{
		if (!Valid(settings) || !Valid(experiments)) return false;
		std::scoped_lock lock(mutex_);
		if (expectedRevision != 0 && expectedRevision != configuration_.revision) return false;
		if (settings == configuration_.settings && experiments == configuration_.experiments) return true;
		if (configuration_.revision == std::numeric_limits<std::uint64_t>::max()) return false;
		auto next = configuration_;
		next.settings = settings; next.experiments = experiments; ++next.revision;
		for (std::uint32_t i = 0; i < 2; ++i) {
			if (!ChangesInput(configuration_, next, i)) continue;
			if (next.inputEpoch[i] == std::numeric_limits<std::uint64_t>::max()) return false;
			++next.inputEpoch[i];
		}
	...
	void Registry::Record(const Observation& observation) noexcept
	{
		try {
			...
			else if (observation.processed) { ++status_.reconstructed; if (observation.bypass) ++status_.bypassed; }
	...
	{
		try { std::scoped_lock lock(mutex_); ++status_.dropped; } catch (...) {}
	}
	void Texture::Abandon() noexcept { (void)resource.Detach(); (void)srv.Detach(); (void)uav.Detach(); }
	void Readback::Abandon() noexcept
	{
		(void)gpu.Detach(); (void)staging.Detach(); (void)uav.Detach(); (void)ready.Detach(); pending = false;
	}
	...
	bool Pipeline::EnsureShaders(ID3D11Device* device, bool diagnostics)
	{
		...
		if (!prepare_) prepare_.Attach(static_cast<ID3D11ComputeShader*>(Util::CompileShader(
			L"Data/Shaders/Upscaling/NeuralRendering/ColorPrepareCS.hlsl", {}, "cs_5_0", "main")));
		if (!reconstruct_) reconstruct_.Attach(static_cast<ID3D11ComputeShader*>(Util::CompileShader(
			L"Data/Shaders/Upscaling/NeuralRendering/ColorReconstructCS.hlsl", {}, "cs_5_0", "main")));
		compileFailed_ = !prepare_ || !reconstruct_;
		if (compileFailed_) return false;
		if (!constants_) {
			D3D11_BUFFER_DESC desc{};
			desc.ByteWidth = sizeof(Constants); desc.Usage = D3D11_USAGE_DEFAULT; desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
		...
		if (diagnostics && !measure_ && !measureCompileAttempted_) {
			...
			try { measure_.Attach(static_cast<ID3D11ComputeShader*>(Util::CompileShader(
				L"Data/Shaders/Upscaling/NeuralRendering/ColorMeasureCS.hlsl", {}, "cs_5_0", "main"))); }
			catch (...) { measure_.Reset(); }
		}
		return true;
	}
	bool Pipeline::Ensure(ID3D11Device* device, Work& work, const ComputeSubrect& roi, DXGI_FORMAT format, bool diagnostics)
	{
		if (!device || !roi.IsValid() || roi.width > D3D11_REQ_TEXTURE2D_U_OR_V_DIMENSION ||
			roi.height > D3D11_REQ_TEXTURE2D_U_OR_V_DIMENSION || !EnsureShaders(device, diagnostics)) return false;
		switch (format) {
		case DXGI_FORMAT_R11G11B10_FLOAT: case DXGI_FORMAT_R16G16B16A16_FLOAT:
		case DXGI_FORMAT_R32G32B32A32_FLOAT: case DXGI_FORMAT_R8G8B8A8_UNORM:
		case DXGI_FORMAT_R10G10B10A2_UNORM: case DXGI_FORMAT_R16G16B16A16_UNORM: break;
		default: return false;
		}
		UINT support = 0;
		if (FAILED(device->CheckFormatSupport(format, &support)) || (support & D3D11_FORMAT_SUPPORT_SHADER_LOAD) == 0 ||
			(support & D3D11_FORMAT_SUPPORT_TYPED_UNORDERED_ACCESS_VIEW) == 0) return false;
		...
		if (diagnostics && measure_ && !work.readbackAttempted) {
			...
			for (auto& readback : work.readbacks) {
				...
				Readback replacement; if (CreateReadback(device, replacement)) readback = std::move(replacement);
			}
		}
		return true;
	}
	void Pipeline::Poll(ID3D11DeviceContext* context, Work& work)
	{
		for (auto& readback : work.readbacks) {
			...
			Measurement measurement{}; measurement.source = readback.source;
			...
			context->Unmap(readback.staging.Get(), 0); Registry::Instance().Record(measurement);
		}
	}
		...
		ID3D11Resource* prepared, ID3D11UnorderedAccessView* preparedUAV, const Configuration& config, Observation observation)
	{
		...
		if (NeedsExposureCapture(config)) {
			...
			o.exposureState = work.exposure.state; o.exposure = work.exposure.evidence;
		} else { o.exposureState = ExposureBindingState::NotRequested; o.exposure = {}; }
		if (work.exposure.resource) o.retainedBytes += 4 * sizeof(float);
		...
		if (o.profile.transform == Transform::Identity) {
			...
			context->CSSetUnorderedAccessViews(0, 1, &preparedUAV, nullptr); context->CSSetShader(prepare_.Get(), nullptr, 0);
		...
		ID3D11ShaderResourceView* neuralSRV, ID3D11ShaderResourceView* preparedSRV, const Configuration& requested)
	{
		...
		if (config.EffectiveMode() == Mode::LegacyRaw && !config.experiments.transportBypass && config.experiments.applyModelEdit) {
			...
			auto* destination = work.result.uav.Get(); context->CSSetShaderResources(0, 4, sources);
			context->CSSetUnorderedAccessViews(0, 1, &destination, nullptr); context->CSSetShader(reconstruct_.Get(), nullptr, 0);
		...
		ID3D11ShaderResourceView* neural, ID3D11ShaderResourceView* prepared)
	{
		...
		context->CopyResource(readback.staging.Get(), readback.gpu.Get()); context->End(readback.ready.Get());
		readback.source = work.observation; readback.pending = true;
	}
