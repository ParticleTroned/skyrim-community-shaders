...
NUMBERED_LEDGER = re.compile(r'^vr-render-scale-ledger-(\d{4,})-(?:pr\d+|history)\.csv$')


def latest_ledger(directory):
    """Select the highest immutable ledger number, rejecting duplicate versions."""
    versions = {}
    for path in directory.glob('vr-render-scale-ledger-*.csv'):
        match = NUMBERED_LEDGER.fullmatch(path.name)
        if not match:
            continue
        number = int(match[1])
        if number in versions:
            raise ValueError(f'duplicate ledger version: {number}')
        versions[number] = path
    if not versions:
        raise ValueError('no numbered render-scale ledger; supply --ledger explicitly')
...
def audit_ledger(ledger, comparison, archives=()):
    """Verify every retained numeric timing, without interpreting missing cells."""
    tables = [read_ledger(path) for path in (ledger, *archives)]
    headers = set()
    for table in tables:
        ...
        for header in table[0][1:]:
            if header in headers:
                raise ValueError("canonical ledger has duplicate run columns across partitions")
            headers.add(header)
    verified, unavailable = [], []
    for side in ("baseline", "candidate"):
        run = comparison[side]
        columns = [(table, i) for table in tables for i, header in enumerate(table[0][1:], 1)
                   if run["runId"] in header]
        if len(columns) != 1:
    ...
    return {"verifiedNumericCells": len(verified), "verified": verified,
            ...
            "historicalRows": len({row[0] for table in tables for row in table[1:]})}
...
def complete_reporting(args, ledger):
    ...
    with exclusive(args.output_root / '.reporting.lock'):
        ...
        try:
            if args.ledger_candidate and NUMBERED_LEDGER.fullmatch(ledger.name):
                raise ValueError('numbered ledgers are immutable; prepare the next numbered snapshot and audit it with --ledger')
            ...
            with exclusive(ledger.with_name(ledger.name + '.reporting.lock')):
                ...
                archives = tuple(getattr(args, 'ledger_archive', ()))
                archive_hashes = fingerprint(archives)
                ...
                audit = audit_ledger(candidate, comparison, archives)
                if sha256(ledger) != original_hash or sha256(candidate) != candidate_hash:
                    raise ValueError('ledger changed during validation')
                if fingerprint(archives) != archive_hashes:
                    raise ValueError('ledger archive changed during validation')
                ...
                audit.update(canonicalLedger=str(ledger), ledgerSha256=sha256(ledger),
                             ledgerArchiveHashes=archive_hashes,
...
def main():
    ...
    parser.add_argument("--ledger-archive", type=Path, action='append', default=[],
                        help="Read-only historical partition; repeat for multiple archives.")
