...
namespace
{
	...
	{
		static CSX::Api::ServiceFoundation foundation({ kToolName, 1, 7, 8 });
		...
		CSX::VRDepthCullingDiagnostics::DrawCategory a_category)
	{
		...
		const auto& reasons = a_snapshot.failOpenByCategory[CSX::VRDepthCullingDiagnostics::ToIndex(a_category)];
		...
		return {
			...
			{ "renderPassMismatch", reasons[CSX::VRDepthCullingDiagnostics::ToIndex(Reason::RenderPassMismatch)] },
			...
			{ "invalidVisibilityContract", reasons[CSX::VRDepthCullingDiagnostics::ToIndex(Reason::InvalidVisibilityContract)] },
			{ "descriptorMismatch", reasons[CSX::VRDepthCullingDiagnostics::ToIndex(Reason::DescriptorMismatch)] },
			{ "bindingRejected", reasons[CSX::VRDepthCullingDiagnostics::ToIndex(Reason::BindingRejected)] },
	...
	json SnapshotJson(const Snapshot& a_snapshot)
	{
		const auto failOpenDraws =
			...
			a_snapshot.invalidVisibilityContract + a_snapshot.descriptorMismatch + a_snapshot.bindingRejected +
		...
		const auto unaccountedBindAttempts = a_snapshot.bindAttempts > accountedBindAttempts ?
		                                         a_snapshot.bindAttempts - accountedBindAttempts :
		                                         0;
		...
		return {
			...
			{ "failOpenReasons", {
									 ...
									 { "renderPassMismatch", a_snapshot.renderPassMismatch },
									 ...
									 { "invalidVisibilityContract", a_snapshot.invalidVisibilityContract },
									 { "descriptorMismatch", a_snapshot.descriptorMismatch },
									 { "bindingRejected", a_snapshot.bindingRejected },
