...
int main()
{
	...
	CHECK(state.clusters[0].stable.recentCount == firstCount);
	StableCharacterMultiRoi independentProjection;
	CHECK(Resolve(reversed, 100, independentProjection, reason).historyKeys == first.historyKeys);
	independentProjection = {};
	const auto laterPlan = Resolve(reversed, 109, independentProjection, reason);
	CHECK(laterPlan.clusterIdentities == first.clusterIdentities);
	CHECK(laterPlan.historyKeys != first.historyKeys);
