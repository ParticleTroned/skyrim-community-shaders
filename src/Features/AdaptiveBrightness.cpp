...
void AdaptiveBrightness::DrawSettings()
{
	...
	if (ImGui::BeginTabBar("##AdaptiveBalanceSections", ImGuiTabBarFlags_None)) {
		if (ImGui::BeginTabItem("Profiles", nullptr, profileSectionFlags)) {
			ImGui::TextWrapped("Tune the lighting, atmosphere, Bloom, and Unified Water appearance used for each time and location type. The current worldspace profile appears alongside the base profiles; all saved scopes can also be managed under Locations.");
			if (!settings.enabled)
