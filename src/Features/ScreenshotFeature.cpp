...
void ScreenshotFeature::DrawSettings()
{
	...
	if (!uiSequenceRequestId.empty() && now >= nextUiSequencePoll) {
		...
		const auto response = HandleApiRequest({
			{ "contractMajor", 1 },
			{ "action", "request_get" },
	...
	StagedPlane& a_plane)
{
	...
	if (a_publicationGeneration == 0 ||
		(a_deviceIdentity != 0 && a_deviceIdentity != reinterpret_cast<std::uintptr_t>(sourceDevice.get()))) {
		return false;
	}
	CS_PROFILE_SCOPE("Screenshot::Stage");
	...
	bool a_ownsQueueSlot)
{
	...
	try {
		...
		if (!StageTexturePlane(
				...
				sourceColorSpace,
				tonemapSceneHdr,
				Hooks::GetCaptureRenderTargetGeneration(),
