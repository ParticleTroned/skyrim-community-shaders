 ...
 */
	...
	namespace
	{
		bool IsObbOcclusionShaderActive()
		{
			auto* globalState = globals::state;
			if (!globalState || !globalState->currentShader) {
				return false;
			}

			const char* fxp = globalState->currentShader->fxpFilename;
			return fxp && std::strcmp(fxp, "OBBOcclusionTesting") == 0;
		}

		bool ShouldOverrideObbDepth()
		{
			...
			return IsObbOcclusionShaderActive();
		}

		bool IsUtilityShadowmaskPassActive()
		{
			auto* globalState = globals::state;
			if (!globalState || !globalState->currentShader) {
				return false;
			...
			if (globalState->currentShader->shaderType.get() != RE::BSShader::Type::Utility) {
				return false;
			}
		...
		bool ShouldOverrideShadowmaskDepth()
		{
			...
			return IsUtilityShadowmaskPassActive();
		}

		class ScopedObbDepthOverride
		{
		public:
			explicit ScopedObbDepthOverride(ID3D11DeviceContext* a_context) :
				context(a_context)
			{
				if (!ShouldOverrideObbDepth()) {
					return;
				}
				...
				if (!overrideSrv) {
					...
					overrideSrv = depthCopy.depthSRV;
				}
				if (!overrideSrv) {
					return;
				}

				context->PSGetShaderResources(17, 1, &previousSrv);
				if (previousSrv == overrideSrv) {
					if (previousSrv) {
						previousSrv->Release();
						previousSrv = nullptr;
					}
				...
				context->PSSetShaderResources(17, 1, &overrideSrv);
				active = true;
			}

			~ScopedObbDepthOverride()
			{
				if (active) {
					context->PSSetShaderResources(17, 1, &previousSrv);
				}
			...
			ScopedObbDepthOverride(const ScopedObbDepthOverride&) = delete;
			ScopedObbDepthOverride& operator=(const ScopedObbDepthOverride&) = delete;
		...
		class ScopedShadowmaskDepthOverride
		{
		public:
			explicit ScopedShadowmaskDepthOverride(ID3D11DeviceContext* a_context) :
				context(a_context)
			{
				if (!ShouldOverrideShadowmaskDepth()) {
					return;
				}

				auto* overrideSrv = static_cast<ID3D11ShaderResourceView*>(nullptr);
				auto& terrainBlending = globals::features::terrainBlending;
				if (terrainBlending.blendedDepthTexture16 && terrainBlending.blendedDepthTexture16->srv) {
					overrideSrv = terrainBlending.blendedDepthTexture16->srv.get();
				} else if (terrainBlending.blendedDepthTexture && terrainBlending.blendedDepthTexture->srv) {
				...
				context->PSGetShaderResources(2, 1, &previousSrv);
				if (previousSrv == overrideSrv) {
					if (previousSrv) {
						previousSrv->Release();
						previousSrv = nullptr;
				...
				context->PSSetShaderResources(2, 1, &overrideSrv);
				active = true;
			}

			~ScopedShadowmaskDepthOverride()
			{
				if (active) {
					context->PSSetShaderResources(2, 1, &previousSrv);
				}
			...
			ScopedShadowmaskDepthOverride(const ScopedShadowmaskDepthOverride&) = delete;
			ScopedShadowmaskDepthOverride& operator=(const ScopedShadowmaskDepthOverride&) = delete;
	...
	struct ID3D11DeviceContext_DrawIndexed
	{
		...
		{
			ScopedObbDepthOverride overrideDepth(This);
			ScopedShadowmaskDepthOverride overrideShadowmask(This);
	...
	{
		static void thunk(ID3D11DeviceContext* This, UINT VertexCountPerInstance, UINT InstanceCount,
