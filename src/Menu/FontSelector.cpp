...
namespace MenuFonts::Selector
{
	namespace
	{
		...
		bool FontFamilyPreviewSelectable(const Util::Fonts::FamilyInfo& family, bool isSelected)
		{
			...
			const auto* previewStyle = ResolveFamilyDefaultStyle(family);
					...
					if (FontFamilyPreviewSelectable(catalog.families[i], isSelected)) {
						...
						if (!isSelected) {
							...
							if (const auto* defaultStyle = ResolveFamilyDefaultStyle(newFamily)) {
								ApplyRoleFontSelection(themeSettings, roleSettings, role, newFamily.name, *defaultStyle, menu);
							} else {
			...
			int& styleIndex)
		{
			...
			for (int i = 0; i < static_cast<int>(selectedFamily.styles.size()); ++i) {
				const bool isSelected = i == styleIndex;
				if (FontStylePreviewSelectable(selectedFamily.styles[i], isSelected) && !isSelected) {
					ApplyRoleFontSelection(
						themeSettings,
						roleSettings,
						role,
						selectedFamily.name,
						selectedFamily.styles[i],
						menu);
				}
				if (isSelected) {
					ImGui::SetItemDefaultFocus();
		...
		size_t roleIndex)
	{
		...
		const auto* selectedFamily = catalog.families.empty() ? nullptr : &catalog.families[familyIndex];
...

		{
			...
			ImGui::TextWrapped("%s", T("menu.settings.font_preview_sample", "The quick brown fox jumps over the lazy dog."));
		}
		...
		if (role == Menu::FontRole::Title) {
			...
			ImGui::SameLine();
			const std::string resetButtonLabel = std::string(T("menu.settings.reset", "Reset")) + "##FeatureHeaderScale";
