...
void PerformanceOverlay::DrawPostFGFrameTimeGraph()
{
	...
	// Draw the graph
	float graphWidth = ImGui::GetWindowWidth() * 0.9f;
	const float graphSpread = std::clamp(
		state.postFGSmoothFrameTimeMs * 0.25f,
		Settings::kGraphMinSpread,
		Settings::kGraphMaxSpread);
	const float graphMin = std::max(
		0.0f,
		state.postFGSmoothFrameTimeMs - graphSpread);
