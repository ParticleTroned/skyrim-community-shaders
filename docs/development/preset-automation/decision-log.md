...
- Evidence: [preset census](./preset-baseline-census.md) found no vendor shader-policy difference outside Upscaling; `Upscaling::GetUpscaleMethod` already chooses `upscaleMethod` when DLSS is available and `upscaleMethodNoDLSS` otherwise; [unified candidates](./unified-preset-candidates-20260817.md) pass the generator's non-writing reproduction and guard checks; the [AMD unified-Quality smoke record](./unified-quality-smoke-20260817.json) converged to FSR4 for both eyes with zero lifecycle failures or fidelity mismatches
...
- Confidence: high for configuration structure and deterministic generation; high for AMD provider fallback in the tested Quality/null-HMD lane; low until native NVIDIA and alternate-runtime execution
