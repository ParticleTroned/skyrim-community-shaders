...
* ``--changed-list-file FILE``: a precomputed list, one path per line. This is
  not whitespace-separated, so feature directory names with spaces survive. CI
  passes the list produced by ``tj-actions/changed-files``. An empty file means
  "unknown change set" and triggers FULL validation for push/release runs.
* git diff, used by default when no list file is given: compares the working
  tree against ``--base`` (default ``HEAD``). An empty diff means "nothing
  changed" and exits 0 without compiling. This is the local-dev path.
...
* a validation config (``.github/configs/**``), CMake, or submodule change
  (``.gitmodules`` or a gitlink bump under ``extern/``), because these can alter
  the entry-point/define set itself;
* a changed shader path that does not map into the AIO shader tree.

hlslkit applies a second layer of the same safety net: if any path we pass is
not found in the shader tree, it falls back to full validation on its own.
...
_FULL_TRIGGERS = (
    re.compile(r"^\.github/configs/"),
    re.compile(r"^CMakeLists\.txt$"),
    re.compile(r"^CMakePresets\.json$"),
    re.compile(r"^cmake/"),
    re.compile(r"^\.gitmodules$"),
    # Submodule pointer bumps surface as gitlink changes under extern/.
    re.compile(r"^extern/"),
)
...
# A shader file we cannot map to the AIO layout, distinct from None
# (not a shader at all).
...
def translate_to_aio(path: str) -> "str | None | _Unmappable":
    ...
    non-shader file, or :data:`UNMAPPABLE` for a shader outside known roots.
    """
    norm = _normalize(path)
...
def classify_changes(changed: list[str]) -> tuple[list[str] | None, str]:
    ...
    validation; otherwise it is the possibly empty list of AIO-relative shader
    paths to validate incrementally.
    """
    ...
    return sorted(set(aio_files)), f"incremental: {len(set(aio_files))} changed shader file(s)"
