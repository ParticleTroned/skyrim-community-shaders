...
namespace NeuralRendering
{
	...
	namespace
	{
			...
			const std::array<RendererApplyArgs, 2>& a_args)
		{
			...
			return "stereo eyes require one device, context, frame, generation, feature mode, reset policy, tuning, trust policy, ordered route pair, and disjoint resources";
		}
	...
	public:
		...
		bool ApplyBatchLocked(
			...
			RendererApplyOutcome& a_outcome,
			std::span<const ValidatedResources> a_validatedResources = {});
		...
		RendererApplyOutcome& a_outcome)
	{
		...
		if (!ApplyBatchLocked(
				std::span(&synchronizedArgs[0], 1),
				leftOutcome,
				std::span<const ValidatedResources>(&resources[0], 1))) {
		...
		const bool rightSucceeded = ApplyBatchLocked(
			std::span(&synchronizedArgs[1], 1),
			rightOutcome,
			std::span<const ValidatedResources>(&resources[1], 1));
		a_outcome.evaluationAttemptedFeatureSlotMask |=
		...
		std::span<const ValidatedResources> a_validatedResources)
	{
		...
		if (a_args.empty() || a_args.size() > 2)
			return false;
		SetActiveFeatureSlotLocked(a_args.front().featureSlot);
		if (!a_validatedResources.empty() &&
			a_validatedResources.size() != a_args.size()) {
			return false;
		}
		...
		if (a_args.size() == 2) {
			std::array<RendererApplyArgs, 2> stereoArgs{
				a_args[0], a_args[1]
			};
		...
		for (std::size_t index = 0; index < a_args.size(); ++index) {
			...
			if (a_validatedResources.empty()) {
				validation = ValidateLocked(args, resources[index]);
			} else {
				resources[index] = a_validatedResources[index];
			}
