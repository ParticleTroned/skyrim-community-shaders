...
namespace SShaderCache
{
	...
	static std::string GetShaderString(ShaderClass shaderClass, const RE::BSShader& shader, uint32_t descriptor, bool hashkey)
	{
		...
		SShaderCache::GetShaderDefines(shader, descriptor, std::span{ defines });
		std::string result;
		if (hashkey)  // generate hashkey so don't include descriptor
			result = fmt::format("{}:{}:{}", sourceShaderFile, magic_enum::enum_name(shaderClass), SShaderCache::MergeDefinesString(defines, true));
		else
			result = fmt::format("{}:{}:{:X}:{}", sourceShaderFile, magic_enum::enum_name(shaderClass), descriptor, SShaderCache::MergeDefinesString(defines, true));
		return result;
	}
		...
		if (useDiskCache && std::filesystem::exists(diskPath)) {
			...
			if (cache.UseFileWatcher()) {
				...
				if (diskCacheOutdated)
					logger::debug("Diskcached shader {} older than {}", SShaderCache::GetShaderString(shaderClass, shader, descriptor, true), std::format("{:%Y%m%d%H%M}", diskCacheTime));
				...
				if (ec) {
					...
					if (std::filesystem::exists(shaderSourcePath)) {
						...
						if (ec) {
							...
							logger::debug("Disk-cached shader {} outdated: source is newer than cache", SShaderCache::GetShaderString(shaderClass, shader, descriptor, true));
						}
					}
				}
			}
...
{
	auto key = SShaderCache::GetShaderString(shaderClass, shader, descriptor, true);
	auto keyWithDescriptor = SShaderCache::GetShaderString(shaderClass, shader, descriptor, false);
	...
	const std::wstring path = SShaderCache::GetShaderPath(
		shader.shaderType == RE::BSShader::Type::ImageSpace ?
			static_cast<const RE::BSImagespaceShader&>(shader).originalShaderName :
			shader.fxpFilename);
	auto pathString = Util::WStringToString(path);
	...
	std::string lowerFilePath = Util::FixFilePath(pathString);
	{
		std::unique_lock lockH{ hlslMapMutex };
		auto it = hlslToShaderMap.find(lowerFilePath);
		hlslRecord newRecord{ key, shader.shaderType.get(), descriptor, shaderClass, SShaderCache::GetDiskPath(shader.fxpFilename, descriptor, shaderClass) };
...
{
	std::string type = SShaderCache::GetTypeFromShaderString(a_key);
...
{
	const std::wstring path = SShaderCache::GetShaderPath(shader.fxpFilename);
...
{
	// Ensure the cache directory exists. ValidateDiskCache() may have just removed it,
	// and individual shader saves only create their own subfolders — without this,
	// SaveFile() writes into a missing directory and silently fails, leaving the cache
	// permanently un-validatable (deleted and fully recompiled on every launch).
	std::error_code ec;
	std::filesystem::create_directories(L"Data\\ShaderCache", ec);
...
bool ShaderCache::UpdateShaderModifiedTime(const std::string& a_type, boolean a_forceUpdate)
{
	...
	std::filesystem::path filePath{ SShaderCache::GetShaderPath(a_type) };
...
std::string ShaderCache::GetDefinesString(const RE::BSShader& shader, uint32_t descriptor)
{
	...
	return SShaderCache::MergeDefinesString(defines, true);
}
...
void ShaderCache::ClearShaderMap(RE::BSShader::Type a_type)
{
	...
	std::unique_lock lockM{ ShaderCache::mapMutex };
	logger::debug("Clearing shaderMap of {}", shaderTypeStr);
	for (auto it = shaderMap.begin(); it != shaderMap.end();) {
		auto typeInKey = SShaderCache::GetTypeFromShaderString(it->first);
...
void ShaderCache::TrackActiveShader(ShaderClass shaderClass, const RE::BSShader& shader, uint32_t descriptor)
{
	...
	if (info.key.empty()) {
		...
		// Construct disk path
		info.diskPath = SShaderCache::GetDiskPath(
			shader.shaderType == RE::BSShader::Type::ImageSpace ?
				static_cast<const RE::BSImagespaceShader&>(shader).originalShaderName :
				shader.fxpFilename,
...
void ShaderCache::ProcessCompilationSet(std::stop_token stoken, ShaderCompilationTask task)
{
	...
	uintmax_t sourceBytes = 0;
	{
		...
		if (sep != std::string::npos) {
			...
			if (auto path = SShaderCache::GetShaderPath(shaderName); !path.empty()) {
				std::error_code ec;
...
{
	return SShaderCache::GetShaderString(shaderClass, shader, descriptor, true);
}
...
void UpdateListener::UpdateCache(const std::filesystem::path& filePath, ShaderCache* cache, bool& clearCache, bool& fileDone)
{
	fileDone = true;
	// Skip directories
