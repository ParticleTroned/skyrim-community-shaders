...
- `SetVRUpscalingTransitionProfile` is intended for interior/exterior transition controllers. It stages Render at Upscale Res, the shared upscaler preset, and the DLSS profile together so Community Shaders can apply one relatch. During the VR save/load grace window, staged Render at Upscale Res and DLSS preset/profile changes stay queued until the post-load runtime reset has completed.
- The individual `SetDLSSMode`, `SetDLSSProfile`, and `SetRenderAtUpscaleResEnabled` setters use the same VR save/load staging when called separately.
