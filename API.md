...
-   `VRUpscalingTransitionProfileDecision::kNoChange` — the requested settings and physical contract already match; do not fade and do not call the setter
-   `VRUpscalingTransitionProfileDecision::kApply` — start the one existing transition fade and immediately call the method-specific atomic setter synchronously
