...
def derive(manifest: dict[str, Any], events: list[dict[str, Any]]) -> Graph:
    ...
    draw_visibility_views_by_submission: dict[str, str | None] = {}
    decisions_by_version_and_object: dict[tuple[str, int], dict[str, Any]] = {}
    candidates: list[dict[str, Any]] = []
    results_by_generation: dict[tuple[int, int], list[dict[str, Any]]] = {}
    eye_submissions_by_frame: dict[int, list[dict[str, Any]]] = {}
    last_event_by_frame: dict[int, dict[str, Any]] = {}
    srv_state: dict[tuple[str, str, int], str | None] = {}
    uav_state: dict[tuple[str, str, int], str | None] = {}
    active_target_bindings: dict[str, str] = {}
    ...
    def active_output_resources(context_id: str) -> set[str]:
        result: set[str] = set()
        binding = target_bindings.get(active_target_bindings.get(context_id, ""))
        if binding:
            ...
            if resource_id:
                result.add(resource_id)
        for (bound_context_id, _, _), view_id in uav_state.items():
    ...
    def clear_conflicting_srvs(context_id: str, output_resources: set[str]) -> int:
        cleared = 0
        for key, view_id in list(srv_state.items()):
            if key[0] == context_id and view_id and view_resource(view_id) in output_resources:
                srv_state[key] = None
                cleared += 1
        return cleared
    ...
    for event in events:
        ...
        context_id = str(event.get("deviceContextObservationId") or "")
        ...
        if event_type == "resource-observed":
            ...
            if binding_id:
                ...
                active_target_bindings[context_id] = binding_id
                hazard_adjustment_count += clear_conflicting_srvs(
                    context_id, active_output_resources(context_id))
        elif event_type == "resource-view-bind":
            key = (context_id, str(payload.get("stage")), int(payload.get("slot", 0)))
            view_id = payload.get("viewObservationId")
            if payload.get("bindingKind") == "shader-resource":
                resource_id = view_resource(view_id)
                if view_id and resource_id and resource_id in active_output_resources(context_id):
                    srv_state[key] = None
                ...
                resource_id = view_resource(view_id)
                if resource_id:
                    hazard_adjustment_count += clear_conflicting_srvs(context_id, {resource_id})
            ...
            engine_frame = payload.get("engineFrame")
            ...
            if isinstance(producer_frame, int) and isinstance(engine_frame, int) and isinstance(object_index, int):
                candidates.append({
                    "event": event,
                    "node": candidate_node,
                ...
                graph.gap(
                    f"Visibility candidate event {sequence} lacks numeric CPU frame, engine frame, or object index.",
                    [candidate_node], True,
                )
        elif event_type == "visibility-result-ready":
            producer_frame = payload.get("producerFrame")
            engine_frame = payload.get("engineFrame")
            if isinstance(producer_frame, int) and isinstance(engine_frame, int):
                results_by_generation.setdefault((producer_frame, engine_frame), []).append(event)
            ...
            version_id = payload.get("resourceVersionObservationId")
            object_index = payload.get("objectIndex")
            if isinstance(version_id, str) and isinstance(object_index, int):
                decisions_by_version_and_object[(version_id, object_index)] = event
            ...
            if event_type == "draw":
                ...
                if submission:
                    ...
                    visibility_slot = submission["payload"].get("slot")
                    draw_visibility_views_by_submission[submission_id] = (
                        srv_state.get((context_id, "vertex", visibility_slot))
                        if isinstance(visibility_slot, int) else None
                    )
                elif submission_id:
                    graph.gap(
                ...
                for (bound_context_id, stage, slot), view_id in sorted(srv_state.items()):
                    if bound_context_id == context_id and stage != "compute" and view_id:
                        read_views.append((view_id, stage, slot))
                for (bound_context_id, stage, slot), view_id in sorted(uav_state.items()):
                    if bound_context_id == context_id and stage == "output-merger" and view_id:
    ...
    for candidate in sorted(candidates, key=lambda item: item["event"]["sequence"]):
        ...
        engine_frame = candidate["engineFrame"]
        object_index = candidate["objectIndex"]
        ready_events = [
            event for event in results_by_generation.get((producer_frame, engine_frame), [])
        ...
        matched_draw = None
        for possible_ready in ready_events:
            possible_version_id = possible_ready.get("payload", {}).get("resourceVersionObservationId")
            for submission in submissions_by_candidate.get((producer_frame, object_index), []):
                submission_event = submission["event"]
                submission_id = submission_event.get("payload", {}).get("submissionObservationId")
                draw = draws_by_submission.get(submission_id)
                if (
                    draw and
                    submission_event["sequence"] > possible_ready["sequence"] and
                    submission_event.get("payload", {}).get("resourceVersionObservationId") == possible_version_id
                ):
                    ready_event = possible_ready
        ...
        if ready_event and matched_submission and matched_draw and version:
            ...
            submission_id = submission_payload.get("submissionObservationId")
            ready_view_id = ready_event.get("payload", {}).get("viewObservationId")
            requested_view_id = submission_payload.get("requestedViewObservationId")
            effective_view_id = submission_payload.get("effectiveViewObservationId")
            draw_view_id = draw_visibility_views_by_submission.get(submission_id)
            effective_view = views.get(effective_view_id)
            effective_view_payload = effective_view and effective_view.get("payload", {})
            version_resource_id = version["payload"].get("resourceObservationId")
            effective_resource_id = effective_view_payload and effective_view_payload.get("resourceObservationId")
            subresources = effective_view_payload.get("subresources", {}) if effective_view_payload else {}
            first_element = subresources.get("firstElement")
            element_count = subresources.get("elementCountOrArraySize")
            object_count = ready_event.get("payload", {}).get("objectCount")
            range_covers_object = (
                isinstance(first_element, int) and isinstance(element_count, int) and
                isinstance(object_count, int) and object_count > 0 and
                first_element == 0 and element_count >= object_count and
                first_element <= object_index < first_element + element_count
            )
            identity_checks = {
                "readyViewObservationId": ready_view_id,
                "requestedViewObservationId": requested_view_id,
                "effectiveViewObservationId": effective_view_id,
                "drawVertexSlotViewObservationId": draw_view_id,
                "versionResourceObservationId": version_resource_id,
                "effectiveViewResourceObservationId": effective_resource_id,
                "firstElement": first_element,
                "elementCount": element_count,
                "objectCount": object_count,
                "objectIndex": object_index,
                "rangeCoversObject": range_covers_object,
            }
            ...
            viable = (
                ...
                submission_payload.get("resourceVersionObservationId") == version_id and
                submission_payload.get("bindingMatches") is True and
                submission_payload.get("slot") == 127 and
                ready_view_id == requested_view_id == effective_view_id == draw_view_id and
                version_resource_id == effective_resource_id and
                range_covers_object
            )
        ...
        observed_decision = decisions_by_version_and_object.get((version_id, object_index)) if version_id else None
        producer_visible = observed_decision and observed_decision.get("payload", {}).get("producerVisible")
        if observed_decision:
            evidence_sequences.append(observed_decision["sequence"])
        ...
        graph.decision_windows.append({
            ...
            "result": result,
            "savings": ["gpu-vertex", "gpu-pixel"] if viable and producer_visible is False else [],
            "evidence": [{
                "captureId": capture_id,
            ...
            "extensions": {
                ...
                "csx.producerVisible": producer_visible,
                "csx.identityChecks": identity_checks,
            },
        })
...
def main() -> int:
    ...
    output = {
        "schema": {"name": "csx.derived-render-graph", "major": 1, "minor": 3, "producerVersion": "resource-versions-2"},
