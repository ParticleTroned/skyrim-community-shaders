...
void LightPicker::Update()
{
	...
	// Throttle hover resolution because effect mode scans the current cell.
	...
	if (hoverMesh.valid) {
		...
		if (!hoverMesh.editorId.empty())
			ImGui::Text(T(TKEY("picked_editor_id"), "EditorID: %s"), hoverMesh.editorId.c_str());
		if (!hoverMesh.modelPath.empty())
			ImGui::Text(T(TKEY("picked_mesh"), "Mesh: %s"), hoverMesh.modelPath.c_str());
		ImGui::Text(T(TKEY("picked_base_form_id"), "Base FormID: 0x%08X"), hoverMesh.baseFormId);
		if (!hoverMesh.refFormEntry.empty())
			ImGui::Text(T(TKEY("picked_reference_form_id"), "Reference FormID: %s"), hoverMesh.refFormEntry.c_str());
		if (!hoverMesh.sourcePlugin.empty())
			ImGui::Text(T(TKEY("picked_plugin"), "Plugin: %s"), hoverMesh.sourcePlugin.c_str());
		ImGui::EndTooltip();
	}

	if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
		// Resolve fresh at click time because the throttled hover hit may trail the cursor.
