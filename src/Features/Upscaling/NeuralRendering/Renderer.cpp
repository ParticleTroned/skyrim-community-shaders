...
namespace NeuralRendering
{
	...
	namespace
	{
		...
		bool HasTextureRegionContract(
			const D3D11_TEXTURE2D_DESC& a_desc,
			std::uint32_t a_width,
			std::uint32_t a_height,
			std::uint32_t a_offsetX,
			std::uint32_t a_offsetY) noexcept
		{
			return a_offsetX <= a_desc.Width &&
			       a_offsetY <= a_desc.Height &&
			       a_width <= a_desc.Width - a_offsetX &&
			       a_height <= a_desc.Height - a_offsetY &&
			       a_desc.MipLevels == 1 &&
			       a_desc.ArraySize == 1 &&
		...
		D3D11_BOX MakeTextureRegion(
			std::uint32_t a_offsetX,
			std::uint32_t a_offsetY,
			std::uint32_t a_width,
		...
		struct alignas(16) CopyDepthGuideConstants
		{
	...
	private:
		...
		Util::LazyShader<ID3D11ComputeShader> copyDepthGuideCS_;
		...
		ValidatedResources& a_resources) const
	{
		...
		const std::array<InputContract, 4> contracts{
			InputContract{ a_args.colorInput, &a_resources.color, a_args.colorWidth, a_args.colorHeight, a_args.inputOffsetX, a_args.inputOffsetY, "color input" },
			InputContract{ a_args.depthGuide, &a_resources.depth, a_args.guideWidth, a_args.guideHeight, a_args.inputOffsetX, a_args.inputOffsetY, "depth guide" },
			InputContract{ a_args.motionVectors, &a_resources.motionVectors, a_args.guideWidth, a_args.guideHeight, a_args.inputOffsetX, a_args.inputOffsetY, "motion vectors" },
			InputContract{ a_args.colorOutput, &a_resources.output, a_args.outputWidth, a_args.outputHeight, a_args.outputOffsetX, a_args.outputOffsetY, "color output" },
		};
		for (const auto& contract : contracts) {
			if (!GetTextureInfo(contract.resource, *contract.info))
				return fail(std::format("{} is not a Texture2D", contract.name));
			if (!HasTextureRegionContract(
					contract.info->desc,
					contract.width,
					contract.height,
					contract.offsetX,
					contract.offsetY)) {
				return fail(std::format(
					"{} does not contain the {}x{} region at ({},{}) within the single-sample, single-subresource DEFAULT contract",
					contract.name,
					contract.width,
					contract.height,
					contract.offsetX,
	...
	bool Renderer::State::CopyDepthLocked(
		const RendererApplyArgs& a_args,
		Slot& a_slot)
	{
		SetActiveFeatureSlotLocked(a_args.featureSlot);
		auto* shader = copyDepthGuideCS_.Get(
			L"Data/Shaders/Upscaling/NeuralRendering/CopyDepthGuideCS.hlsl",
			{},
			"cs_5_0",
			"main",
			"NeuralRendering::CopyDepthGuideCS");
		if (!shader)
			return false;

		if (!copyDepthGuideCB_) {
			D3D11_BUFFER_DESC desc{};
			desc.ByteWidth = sizeof(CopyDepthGuideConstants);
			desc.Usage = D3D11_USAGE_DEFAULT;
			desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
			if (FAILED(a_args.device->CreateBuffer(
					&desc, nullptr, &copyDepthGuideCB_))) {
				return false;
			}
			Util::SetResourceName(
				copyDepthGuideCB_.Get(),
				"NeuralRendering::CopyDepthGuideConstants");
		}
		...
		a_args.context->CSSetShader(shader, nullptr, 0);
		const CopyDepthGuideConstants constants{
			.sourceOffsetX = a_args.inputOffsetX,
			.sourceOffsetY = a_args.inputOffsetY,
		};
		a_args.context->UpdateSubresource(
			copyDepthGuideCB_.Get(), 0, nullptr, &constants, 0, 0);
		auto* source = a_args.depthGuideSRV;
		auto* destination = a_slot.depth.uav11.Get();
			...
			0, 1, &destination, nullptr);
		{
			...
			a_args.context->Dispatch(
				(a_args.guideWidth + 7u) / 8u,
				(a_args.guideHeight + 7u) / 8u,
				1);
		}
		return true;
	}
		...
		RendererApplyOutcome& a_outcome)
	{
		...
		SetRequestTelemetryLocked(a_args);
