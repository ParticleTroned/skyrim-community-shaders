...
namespace NeuralRendering::Color
{
	...
	namespace
	{
		...
		bool CreateValueTexture(ID3D11Device* device, ExposureBinding& value, bool uav, ComPtr<ID3D11UnorderedAccessView>& view)
		{
			...
			if (FAILED(device->CreateTexture2D(&desc, &initial, &texture)) ||
				...
				return false;
			Util::SetResourceName(texture.Get(), "NeuralRendering::ColourExposureSnapshot");
			Util::SetResourceName(srv.Get(), "NeuralRendering::ColourExposureSnapshot SRV");
			if (output)
				Util::SetResourceName(output.Get(), "NeuralRendering::ColourExposureSnapshot UAV");
	...
	struct ExposureCapture::State
	{
		...
		void Capture(ID3D11DeviceContext* c, RE::BSShader* producer, std::optional<ExposureDrawKind> kind)
		{
			...
			{
				CS_GPU_PASS("Upscaling::NRColorExposureCapture");
				c->Dispatch(1, 1, 1);
			}
			guard.Unbind();
			...
			if (!e.staging) {
				...
				Util::SetResourceName(e.staging.Get(), "NeuralRendering::ColourExposureReadback");
			}
			if (!e.ready) {
				...
				Util::SetResourceName(e.ready.Get(), "NeuralRendering::ColourExposureReady");
			}
				...
				if (cb.ByteWidth >= e.gammaOffset + sizeof(float) && cb.ByteWidth <= D3D11_REQ_CONSTANT_BUFFER_ELEMENT_COUNT * 16u) {
					if (!e.gammaStaging || e.gammaBytes != cb.ByteWidth) {
						...
						if (SUCCEEDED(d->CreateBuffer(&stage, nullptr, &e.gammaStaging))) {
							...
							Util::SetResourceName(e.gammaStaging.Get(), "NeuralRendering::ColourGammaReadback");
						}
					}
					if (e.gammaStaging) {
