...
#else
        depthSamples[0] = SharedData::DepthTexture.Load(int3(int2(screenPosition.xy + float2(0.5f, 0.5f)), 0)).x;
#endif

        shadowSamples[0] = ScreenSpaceShadowsTexture.Load(int3(int2(screenPosition.xy + float2(0.5f, 0.5f)), 0)).x;
