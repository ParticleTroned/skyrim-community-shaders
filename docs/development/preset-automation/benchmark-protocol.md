...
That restriction is historical to the source snapshot used by the run. Current Release+DevBench builds permit only the bounded production `communityshaders.capture` start/cancel controls at Info; diagnostic texture readback and unrelated behavioural or testing mutations retain their Developer Mode requirement. Future Info candidates should therefore collect a separate, matched visual pass without changing log level.
