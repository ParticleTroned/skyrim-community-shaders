...
#		define LIGHTING_LANDSCAPE_BLEND_ONE_LAYER_PBR(TILE, COLOR_TEX, COLOR_SAMP, NORM_TEX, NORM_SAMP, RMAOS_TEX, RMAOS_SAMP, PBR_PARAMS3, GLINT_PARAMS, WEIGHT) \
			...
			{                                                                                                                                                      \
				float weight = WEIGHT;                                                                                                                             \
				float4 landColor = SampleTerrainColor(COLOR_TEX, COLOR_SAMP, uv, sharedOffset);                                                                    \
				...
				float4 landNormal = SampleTerrainNormal(NORM_TEX, NORM_SAMP, uv, sharedOffset);                                                                    \
				float3 landNormalRGB = landNormal.rgb;                                                                                                             \
				float landNormalAlpha = landNormal.a;                                                                                                              \
				float4 landRMAOS;                                                                                                                                  \
				[branch] if (LandscapeLayers::PbrTileUsesFullPBR(TILE))                                                                                            \
				{                                                                                                                                                  \
					landRMAOS = SampleTerrainMaterialData(RMAOS_TEX, RMAOS_SAMP, uv, sharedOffset) * float4((PBR_PARAMS3).x, 1, 1, (PBR_PARAMS3).z);               \
