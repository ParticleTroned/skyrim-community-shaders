...
namespace
{
	...
	std::string_view CompatibilityPhaseName(CSX::ShaderCompatibilityAPI::Phase a_phase)
	{
		using Phase = CSX::ShaderCompatibilityAPI::Phase;
		switch (a_phase) {
		case Phase::kAcceptingRegistrations: return "accepting-registrations";
		case Phase::kFrozen: return "frozen";
		default: return "unknown";
		}
	}

	std::string_view CompatibilityScopeName(CSX::ShaderCompatibilityAPI::ScopeKind a_kind)
	{
		using ScopeKind = CSX::ShaderCompatibilityAPI::ScopeKind;
		switch (a_kind) {
		case ScopeKind::kShaderFamily: return "shader-family";
		case ScopeKind::kShaderSource: return "shader-source";
	...
	json BuildShaderCompilationProvenance()
	{
		...
		auto& registry = CSX::Api::GetShaderCompatibilityRegistry();
		const auto registrySnapshot = registry.GetSnapshot();
		json registrations = json::array();
		for (std::uint32_t index = 0; index < registrySnapshot.registrationCount; ++index) {
			CSX::Api::ShaderCompatibilityRegistration registration;
			if (!registry.GetRegistration(index, registration))
				continue;
			json scopes = json::array();
			for (const auto& scope : registration.scopes) {
				scopes.push_back({
					{ "kind", CompatibilityScopeName(scope.kind) }, { "value", scope.value },
				});
			}
			registrations.push_back({
				{ "handle", registration.handle }, { "identity", registration.identity },
				{ "owner", registration.owner }, { "displayVersion", registration.displayVersion },
				...
				{ "resourceFingerprint", registration.resourceFingerprint },
				{ "canonical", registration.canonical }, { "canonicalDigest", registration.digest },
		...
		return {
			...
			{ "compatibilityRegistry", {
				{ "phase", CompatibilityPhaseName(registrySnapshot.phase) },
				{ "revision", registrySnapshot.revision },
				{ "registrationCount", registrySnapshot.registrationCount },
				{ "compatibilitySetDigest", registrySnapshot.compatibilitySetDigest.empty() ?
					json(nullptr) : json(registrySnapshot.compatibilitySetDigest) },
				{ "complete", registrations.size() == registrySnapshot.registrationCount },
				{ "registrations", std::move(registrations) },
			} },
			{ "qualification",
				"globalCompileState excludes the shader-specific compatibility requirement set; "
				"derive that set from the retained registrations and the observed shader family/source" },
		};
	}
