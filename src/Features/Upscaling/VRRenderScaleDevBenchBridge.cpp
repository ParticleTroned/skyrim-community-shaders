...
namespace
{
  ...
  "inputSchema":{
    ...
    "properties":{
      ...
      "mode":{"oneOf":[{"type":"string","enum":["full_resolution","foveated","reduced_resolution"]},{"type":"integer","minimum":0,"maximum":2}],"description":"NR execution mode: full output resolution, shared FOV masks, or Renderscale NR before DLSS. In VR, foveated and reduced_resolution automatically require and use the shared FOV mask. Flat reduced_resolution covers the mono image. Character selection combines with every mode; runtime admission remains separate from menu configuration."},
      "fovOnly":{"type":"boolean","description":"Saved optional FOV restriction for Full resolution. Foveated and VR Renderscale NR before DLSS always use the mask even when this preference is false. Requires configured and enabled VR FOV; flat has no FOV mask. Full resolution shares mask geometry, offsets and feather."},
	...
	json NeuralRenderingStatusJson(const Upscaling& a_upscaling)
	{
		...
		const bool fovRequired = NeuralRendering::RequiresFoveatedMask(a_upscaling.GetNeuralRenderingMode(), a_upscaling.settings.neuralRenderingFovOnly, globals::game::isVR);
	...
	json BuildRenderScaleResult(const json& a_args)
	{
		...
		if (action == "nr_readiness") {
			...
			return RunOnMainThread([]() {
				...
				require(!NeuralRendering::RequiresFoveatedMask(upscaling.GetNeuralRenderingMode(), upscaling.settings.neuralRenderingFovOnly, globals::game::isVR) ||
